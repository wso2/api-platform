...
function deleteProvider(authToken, organizationId, targetProviderId) {
  ...
  return requestWithAuth(authToken, {
    method: 'DELETE',
    url: `/proxy/api/v0.9/llm-providers/${encodeURIComponent(targetProviderId)}?organizationId=${encodeURIComponent(organizationId)}`,
...
function deleteTemplateVersion(authToken, organizationId, templateId) {
  return requestWithAuth(authToken, {
    method: 'DELETE',
    url: `/proxy/api/v0.9/llm-provider-templates/${encodeURIComponent(templateId)}`,
    failOnStatusCode: false,
  }).then((response) => {
