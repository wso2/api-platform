...
function deleteLinkedProxies(authToken, organizationId, providerId) {
  ...
  return requestWithAuth(authToken, {
    url: `/proxy/api/v0.9/llm-providers/${encodeURIComponent(providerId)}/llm-proxies?organizationId=${encodeURIComponent(organizationId)}`,
...
function deleteProject(authToken, projectId) {
  return requestWithAuth(authToken, {
    method: 'DELETE',
    url: `/proxy/api/v0.9/projects/${encodeURIComponent(projectId)}`,
    failOnStatusCode: false,
  }).then((response) => {
