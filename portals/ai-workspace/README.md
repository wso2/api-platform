...
| Mode | When to use |
|---|---|
| `basic` | Quickstart / air-gapped — admin credentials generated by `setup.sh` (see `keys.env`) |
...
file-based quickstart (the admin user generated by `setup.sh`).
...
`setup.sh` generates self-signed certificates into `certs/`. To remove the
browser trust warning, replace them with certificates from your own CA — same
file names:
...
Then restart the stack: `docker compose --env-file keys.env up -d`
...
|---|---|---|
| **Platform API** — `APIP_CP_ENCRYPTION_KEY`, `APIP_CP_AUTH_JWT_SECRET_KEY` | Generated into `keys.env` | Manage as real secrets; prefer mounting files and referencing them with `{{ file "..." }}` in the config TOML |
| **Platform API** — admin credentials | Generated into `keys.env` (bcrypt hash); password printed once | Use OIDC (`auth.idp`) instead of file-based auth |
| **TLS certificates (both services)** | Self-signed pairs in `certs/` | Certificates from your CA (same file names), or terminate TLS at an ingress and disable the listeners' TLS |
| **Upstream trust (BFF → Platform API)** | The generated platform-api cert is mounted as a CA bundle (`[platform_api] ca_file`) | Point `ca_file` at your CA bundle; never use `tls_skip_verify` in production |
