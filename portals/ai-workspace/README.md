...
`setup.sh` generates one self-signed certificate into `resources/certificates/`,
shared by both services. To remove the browser trust warning, replace it with a
certificate from your own CA — same file names, and the SAN list must cover both
...
docker-compose mounts this one directory read-only into both containers
(`/etc/platform-api/tls` and `/etc/ai-workspace/tls`), so replacing the two
files is all it takes.
