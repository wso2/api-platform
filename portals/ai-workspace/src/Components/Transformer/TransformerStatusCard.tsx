...
export type TransformerStatusCardProps = {
  ...
  onRemove?: () => void;
  /**
   * Drops the card entirely where the formats already match and nothing is
   * attached. For a surface still being filled in, a card that only says a
   * step is unnecessary is a step in the way; where a proxy is being reviewed
   * rather than built, the same sentence is worth having, so this is opt-in.
   */
...
export default function TransformerStatusCard({
  ...
  // `none` is reached only where the formats were compared and found to match
  // with nothing attached. A translator that is attached but unnecessary
  // resolves to `manual` and still shows, because it is on the proxy and can
  // be taken off.
  if (hideWhenNotNeeded && resolution.status === 'none') {
    return null;
  }
