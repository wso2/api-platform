...
export type TransformerPoliciesState = {
  ...
  /**
   * False until the catalogue has answered with something to match against.
   *
   * Stays false when every catalogue read failed, because the alternative is
   * worse than waiting: consumers pass this straight to the transformer
   * resolver, and an empty catalogue reported as loaded makes it call every
   * attached translator invalid and every provider unresolved — an outage
   * rendered as a configuration fault on every proxy at once. `reload` retries.
   */
  isLoaded: boolean;
  isLoading: boolean;
  error: Error | null;
...
export default function useTransformerPolicies(): TransformerPoliciesState {
  ...
  const load = useCallback(async () => {
    ...
    setIsLoading(false);
    // Not loaded where nothing was read. Saying otherwise would hand the
    // resolver an empty catalogue as though it were the whole truth.
    setIsLoaded(!catalogueUnavailable);
  }, []);
