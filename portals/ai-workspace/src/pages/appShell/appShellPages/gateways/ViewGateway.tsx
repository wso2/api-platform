...
export default function ViewGateway() {
  ...
  const [connectionStatusDots, setConnectionStatusDots] = useState("");
...
ENVFILE`;
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      ...
      setConnectionStatusDots(frames[frameIndex]);
    }, 450);
...
GATEWAY_REGISTRATION_TOKEN=${registrationToken || ""}`;
  ...
  const renderGatewayConnectionStatus = () => {
    if (!gateway) {
      return null;
    }
    ...
    return (
      <Stack
        ...
        spacing={1}
        alignItems="center"
        sx={{ mt: 1.5, color: "info.main" }}
      >
        <CircularProgress size={14} thickness={6} color="inherit" />
        <Typography variant="body2" sx={{ fontWeight: 600, color: "inherit" }}>
          Waiting for gateway to connect{connectionStatusDots}
        </Typography>
      </Stack>
    );
  };
  ...
  const descriptionText = gateway?.description?.trim() || "";
  ...
  return (
    <PageContent fullWidth>
                ...
                {registrationToken ? (
                  ...
                  <>
                    <Typography
                      ...
                      sx={{ mb: 2 }}
                    >
                      Your existing gateway installation can still use the
                      previously generated token. If you need a new token and an
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Reconfiguring will revoke the previously generated token for this
            gateway. Any local gateway instance still using that token must be
            updated with the new token. Do you want to continue?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
