...
export function normalizeAccessControlMode(mode?: string): 'allow_all' | 'deny_all' | '' {
  if (!mode) return '';
...
export function filterOpenApiSpecByAccessControl(
  ...
  Object.entries(rootPaths as Record<string, unknown>).forEach(([path, pathItemUnknown]) => {
