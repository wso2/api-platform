...
export function DefinitionPanel({ api }: { api: RestApi }) {
  ...
  const isSaving = putOpenApi.isPending || updateApi.isPending;
  ...
  const handleSave = () => {
    ...
    // Derive operations from spec. Since operationId is not stored in api.operations,
    // method+path is used to uniquely identify an operation.
    //
    // Treat any changed method or path as a new operation (old one implicitly
    // absent — its policies are dropped).
    ...
    >
      ...
      {isDirty && (
        ...
        >
          ...
          <Button loading={isSaving} onClick={handleSave} size="small" variant="contained">
