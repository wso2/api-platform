...
export type EditGraphqlApiFormProps = {
  ...
  isSaving: boolean;
  onCancel: () => void;
  onSubmit: (values: GraphqlApiBasicInfoFormValues) => void;
};
...
const toFormValues = (api: GraphQLApiDetail): GraphqlApiBasicInfoFormValues => ({
  context: api.context ?? '',
 ...
 * `GraphqlApiEditPage`, so this component renders from props alone.
 */
export const EditGraphqlApiForm = (props: EditGraphqlApiFormProps) => {
  ...
  const [values, setValues] = useState<GraphqlApiBasicInfoFormValues>(() =>
    toFormValues(props.api),
  );
  ...
  const setField = <K extends keyof GraphqlApiBasicInfoFormValues>(
    key: K,
    value: GraphqlApiBasicInfoFormValues[K],
