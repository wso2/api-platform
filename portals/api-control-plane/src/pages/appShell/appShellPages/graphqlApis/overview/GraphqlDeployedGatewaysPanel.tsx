...
/** Fork of `apis/overview/DeployedGatewaysPanel.tsx` for a GraphQL API — only
 * the deployments-module import and the Deploy-page link differ. */
export function GraphqlDeployedGatewaysPanel({ gateways, deployments }: Props) {
      ...
      {gatewayDeployments.length > 5 && (
        <Box sx={{ borderTop: '1px solid', borderColor: 'divider', p: 1, textAlign: 'center' }}>
          <Button
            ...
            size="small"
            to={routes.graphqlApiDeploy(orgHandle, projectHandler, graphqlApiHandler)}
          >
