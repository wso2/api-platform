...
export const apiInitials = (displayName?: string): string =>
  (displayName ?? '').replace(/[^\p{L}\p{N}]/gu, '').slice(0, 2).toUpperCase();
...
): { color: ChipColor; label: string } => DEPLOYMENT_META[state];
...
/**
 * Client-side search over the fields a user would type: name, handle, context
 * and description. The list endpoint has no query filter, so this narrows the
 * page already in cache rather than issuing a request per keystroke.
 */
export const filterRestApis = (apis: RestApi[], search: string): RestApi[] => {
  const term = search.trim().toLowerCase();
  if (!term) return apis;
  return apis.filter((api) =>
    [api.displayName, api.id, api.context, api.description].some((field) =>
