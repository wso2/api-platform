...
describe('parseContractResources', () => {
  ...
  it('reads Swagger v2 and YAML documents', () => {
    const yamlDoc = ['swagger: "2.0"', 'paths:', '  /items:', '    get: {}', '    put: {}'].join(
      '\n',
    );
...
describe('discoverBackendResources', () => {
  ...
  it('returns the first candidate that resolves to resources', async () => {
    server.use(
      ...
      http.get(`${BASE}/openapi.yaml`, () => HttpResponse.text('paths:\n  /orders:\n    get: {}')),
    );
    expect(await discoverBackendResources(BASE)).toEqual([{ method: 'GET', path: '/orders' }]);
  });

  it('returns [] when a contract is reachable but declares no resources', async () => {
    server.use(
      http.get(`${BASE}/openapi.json`, () => HttpResponse.json({ openapi: '3.0.0', paths: {} })),
