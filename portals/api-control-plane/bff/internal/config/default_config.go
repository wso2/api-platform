...
// and are enforced by Config.validate instead. RuntimeConfig is populated by
// Load, not here.
	...
	return &Config{
		Server: ServerConfig{
			...
			HTTPS: HTTPSListener{
				...
				CertFile: "/etc/api-control-plane/tls/cert.pem",
				KeyFile:  "/etc/api-control-plane/tls/key.pem",
			},
		},
		...
		Session: SessionConfig{
			...
			Cookie: CookieConfig{
				Name:     "_api_control_plane_session",
				Secure:   true,
				SameSite: "lax",
			},
		},
		Auth: AuthConfig{
			...
			// Defaults mirror the Platform API's own claim_mappings defaults so the
			// two agree out of the box; override on both sides together when an IdP
