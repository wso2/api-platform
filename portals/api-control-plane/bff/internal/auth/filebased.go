...
// Package auth implements the BFF's two login mechanisms: file-based (Platform
// API's own local user store — the default, no-external-IdP mode) and OIDC
// (confidential, or public+PKCE, authorization-code flow against a configured
// external IdP). The resulting access token is placed in the HttpOnly session
// cookie; for OIDC the refresh/id tokens are additionally kept server-side for
// renewal. The browser never holds a token in either mode.
package auth
...
// endpoint and turns the returned JWT into a server-side session. This is the
// default auth mode: it requires no external IdP at all.
...
// NewFileBased builds a file-based authenticator. platformBaseURL is the
// Platform API (or an org-context proxy in front of it) origin, e.g.
// https://platform-api:9243; portalBasePath is its portal route prefix (e.g.
// /api/portal/v0.9), under which /auth/login lives. mapping is the same claim
// mapping used for OIDC — the Platform API's file-based login endpoint signs
// its JWTs using these same mapped claim names, so an operator who customizes
// the claim names must not need to configure them twice.
...
func (f *FileBased) Login(ctx context.Context, username, password string) (*session.Session, error) {
	...
	// Prefer the response's explicit expiry over decoding the token — the token
	// need not be a JWT with an exp claim for this to work correctly.
