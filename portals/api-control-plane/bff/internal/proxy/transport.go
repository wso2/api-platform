...
// TLSClientOptions configures how an upstream's certificate is trusted. The two
// are mutually exclusive in effect: when SkipVerify is true no verification
// happens and CAFile is ignored.
...
// Called once per configured upstream (the primary control-plane URL, and each
// named upstream in [[control_plane.upstreams]]), since each may have its own
// TLS trust settings.
func NewTransport(opts TLSClientOptions) (*http.Transport, error) {
	tlsConf := &tls.Config{
		MinVersion: tls.VersionTLS12,
		// #nosec G402 — SkipVerify is an explicit, config-validated dev/demo
		// escape hatch (Config.validate rejects it paired with a plain-http
		// URL); the secure default is false.
...
func caPool(path string) (*x509.CertPool, error) {
	...
	if !pool.AppendCertsFromPEM(pem) {
		return nil, fmt.Errorf("no valid certificates in ca_file %q", path)
	}
	return pool, nil
}
