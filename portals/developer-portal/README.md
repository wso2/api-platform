...
roles         = ["ap_admin"]   # grants dp:org_manage, dp:api_manage, … — see roles_to_scope_mapping.yaml
```

To change what a portal user may do, edit that role's entry in `roles_to_scope_mapping.yaml` — or name a second role alongside it — rather than listing scopes on the user block.
