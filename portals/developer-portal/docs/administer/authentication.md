...
3. The IDP issues an org-scoped token. On every authenticated request, the portal checks that the token's org claim (`orgIDClaim`) matches the org's `ORGANIZATION_IDENTIFIER`. A mismatch returns a 403.
