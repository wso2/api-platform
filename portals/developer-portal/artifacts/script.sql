...
ALTER TABLE "DP_API_METADATA"
ADD COLUMN IF NOT EXISTS "AGENT_VISIBILITY" VARCHAR(32) NOT NULL DEFAULT 'VISIBLE'
    CHECK ("AGENT_VISIBILITY" IN ('VISIBLE', 'HIDDEN'));
...
ALTER TABLE "DP_SUBSCRIPTION_POLICY"
    ...
    ADD COLUMN IF NOT EXISTS "REF_ID" VARCHAR(256);
...
ALTER TABLE "DP_API_SUBSCRIPTION"
    ...
    ADD COLUMN IF NOT EXISTS "SUB_TOKEN" VARCHAR(512),
    ADD COLUMN IF NOT EXISTS "STATUS" VARCHAR(32) NOT NULL DEFAULT 'ACTIVE';

ALTER TABLE "DP_API_SUBSCRIPTION" DROP CONSTRAINT IF EXISTS "DP_API_SUBSCRIPTION_APP_ID_ORG_ID_API_ID_POLICY_ID_key";
...
-- ─────────────────────────────────────────────────────────
-- Key Manager: per-org OAuth2 Authorization Server configs
-- ─────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS "DP_KEY_MANAGER" (
    "KM_ID" UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    "ORG_ID" VARCHAR(256) NOT NULL,
    "NAME" VARCHAR(256) NOT NULL,
...
ALTER TABLE "DP_APP_KEY_MAPPING"
    ADD CONSTRAINT "FK_APP_KEY_MAPPING_KM_ID"
