...
// organizations.id (the REST `id`/handle) is not the internal uuid stored as
// org_uuid on events etc. — resolve it once per org via its unique handle.
async function findOrgUuidByHandle(handle) {
    const rows = await query('SELECT uuid FROM organizations WHERE handle = $1', [handle]);
    return rows[0]?.uuid;
}

// Columns per database/schema.postgres.sql: events(uuid, type, org_uuid,
...
async function findEvents({ orgUuid, type, aggregateUuid, since }) {
    ...
    return query(`SELECT * FROM events ${where} ORDER BY occurred_at DESC`, params);
}

// Columns per database/schema.postgres.sql: event_deliveries(uuid, event_uuid,
...
async function findDeliveries({ eventUuid, subscriberId }) {
    ...
    const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';
    return query(`SELECT * FROM event_deliveries ${where}`, params);
}
