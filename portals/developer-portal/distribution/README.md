...
| `[[platform_api.auth.file.users]]` | Local user credentials — `username`/`password_hash` resolved from `setup.sh`'s env vars; `roles` names one or more entries in `resources/roles_to_scope_mapping.yaml`, which is where that user's scopes come from | admin, generated by `setup.sh` |
| `[platform_api.auth.authorization].roles_to_scope_mapping` | Path to the mounted `resources/roles_to_scope_mapping.yaml` — edit that file to change what a role grants | `/etc/platform-api/roles_to_scope_mapping.yaml` |
