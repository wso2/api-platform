...
```bash
curl -k -X POST \
  "https://localhost:9543/api-portal/api/v0.9/apis/{apiId}/assets" \
...
```bash
curl -k "https://localhost:9543/api-portal/api/v0.9/apis/{apiId}/assets?type=document&fileName=getting-started.md" \
  -H "Authorization: Bearer $TOKEN"
```
