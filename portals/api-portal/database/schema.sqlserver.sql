...
CREATE TABLE dbo.organizations (
    -- In a multi-portal scenario the same logical org (same handle) has a distinct UUID per
    -- portal; this uuid identifies the org within this portal only, not across portals.
...
CREATE INDEX idx_organization_asset_org_uuid ON dbo.organization_assets(org_uuid, portal_id);
...
CREATE INDEX idx_organization_asset_view_uuid ON dbo.organization_assets(view_uuid, portal_id);
...
CREATE UNIQUE INDEX uq_view_label_mappings_label_view ON dbo.view_label_mappings(label_uuid, view_uuid, portal_id);
...
CREATE UNIQUE INDEX uq_api_label_mappings_label_api ON dbo.api_label_mappings(label_uuid, api_uuid, portal_id);
...
CREATE UNIQUE INDEX uq_api_tag_mappings_tag_api ON dbo.api_tag_mappings(tag_uuid, api_uuid, portal_id);
...
CREATE UNIQUE INDEX uq_api_subscription_plan_mappings_plan_api ON dbo.api_subscription_plan_mappings(plan_uuid, api_uuid, portal_id);
...
CREATE INDEX idx_application_org_created_by ON dbo.applications(org_uuid, created_by, portal_id);
...
CREATE INDEX idx_subscription_org_created_by ON dbo.subscriptions(org_uuid, created_by, portal_id);
...
CREATE INDEX idx_subscription_org_api_uuid ON dbo.subscriptions(org_uuid, api_uuid, portal_id);
...
CREATE INDEX idx_api_key_org_api_uuid ON dbo.api_keys(org_uuid, api_uuid, portal_id);
...
CREATE TABLE dbo.oauth2_consumer_keys (
    uuid VARCHAR(40) NOT NULL,
    org_uuid VARCHAR(40) NOT NULL,
    portal_id VARCHAR(255) NOT NULL DEFAULT 'portal_id',
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'idx_oauth2_consumer_key_org_created_by' AND object_id = OBJECT_ID(N'dbo.oauth2_consumer_keys'))
CREATE INDEX idx_oauth2_consumer_key_org_created_by ON dbo.oauth2_consumer_keys(org_uuid, created_by, portal_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'idx_oauth2_consumer_key_km_id' AND object_id = OBJECT_ID(N'dbo.oauth2_consumer_keys'))
CREATE INDEX idx_oauth2_consumer_key_km_id ON dbo.oauth2_consumer_keys(key_manager_id, portal_id);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'idx_oauth2_consumer_key_status' AND object_id = OBJECT_ID(N'dbo.oauth2_consumer_keys'))
CREATE INDEX idx_oauth2_consumer_key_status ON dbo.oauth2_consumer_keys(status);
...
IF OBJECT_ID(N'dbo.oauth2_consumer_key_app_mappings', N'U') IS NULL
CREATE TABLE dbo.oauth2_consumer_key_app_mappings (
    key_uuid VARCHAR(40) NOT NULL,
    app_uuid VARCHAR(40) NOT NULL,
    portal_id VARCHAR(255) NOT NULL DEFAULT 'portal_id',
    created_by VARCHAR(255) NOT NULL,
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'idx_oauth2_key_app_mapping_app' AND object_id = OBJECT_ID(N'dbo.oauth2_consumer_key_app_mappings'))
CREATE INDEX idx_oauth2_key_app_mapping_app ON dbo.oauth2_consumer_key_app_mappings(app_uuid);
...
-- safe defaults.
IF OBJECT_ID(N'dbo.key_manager_configurations', N'U') IS NULL
CREATE TABLE dbo.key_manager_configurations (
    ...
    FOREIGN KEY (portal_id, key_manager_uuid) REFERENCES dbo.key_managers(portal_id, uuid) ON DELETE CASCADE,
    FOREIGN KEY (portal_id, org_uuid) REFERENCES dbo.organizations(portal_id, uuid) ON DELETE NO ACTION
);

-- One configuration per key manager, and the lookup every read uses.
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'idx_key_manager_config_org' AND object_id = OBJECT_ID(N'dbo.key_manager_configurations'))
CREATE INDEX idx_key_manager_config_org ON dbo.key_manager_configurations(org_uuid, portal_id);
...
CREATE INDEX idx_api_workflow_view_uuid ON dbo.api_workflows(view_uuid, portal_id);
