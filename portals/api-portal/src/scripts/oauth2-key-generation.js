...
async function removeApplicationKeys(applicationId, keyMappingId, keyType) {
    ...
    try {
        const response = await fetch(apiPortalApi.root(`/applications/${applicationId}/oauth-keys/${keyMappingId}`), {
            method: 'DELETE',
            headers: {
...
async function generateOauthKey(formId, appId, keyMappingId, keyManager, clientName, clientSecret, subscribedScopes, keyType) {
    ...
    try {
        const response = await fetch(apiPortalApi.root(`/applications/${appId}/oauth-keys/${keyMappingId}/generate-token`), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
