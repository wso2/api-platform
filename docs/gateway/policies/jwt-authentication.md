...
When using **application-level subscriptions**, the JWT auth policy should also map an application identifier claim (for example, `azp` or `client_id`) into the internal metadata key `x-wso2-application-id`. This metadata is then used by the `subscription-validation` api-level policy at the gateway to enforce that:
