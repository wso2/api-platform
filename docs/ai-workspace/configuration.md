...
### `[ai_workspace.oidc.claim_mappings]` — which token claim carries each field

This table mirrors the Platform API's `[platform_api.auth.idp.claim_mappings]` key for key, and the two must agree: both services read the same claims out of the same token.
...
`[ai_workspace.oidc] redirect_url` and `post_logout_redirect_url` must be registered as authorized redirect
...
The remaining tables (`[ai_workspace.tls]`, `[ai_workspace.session]`, `[ai_workspace.cookie]`) and the `[ai_workspace]` listener keys are documented inline in
