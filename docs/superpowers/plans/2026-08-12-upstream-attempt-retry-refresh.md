...
- [ ] **Step 1: Extend the schema.** In `management-openapi.yaml`, find the `Resilience` schema (currently `timeout`/`idleTimeout` only, per the existing description block) and add a `retry` property:
