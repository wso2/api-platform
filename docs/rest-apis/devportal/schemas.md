...
|Name|Type|Required|Restrictions|Description|
|---|---|---|---|---|
|id|string|false|none|Optional handle for the webhook subscriber (unique per org), stored as-is. When omitted, one is generated by slugifying displayName; if that handle is taken a numeric suffix is appended (`-2`, `-3`), falling back to a random suffix, so a duplicate display name never fails the request. Supply it only when you need a specific, stable identifier — it is the id used in the resource path, and a collision on a handle you supplied yourself is reported as 409 rather than renamed.|
|displayName|string|true|none|Display name. Also the basis for the generated handle when id is omitted.|
