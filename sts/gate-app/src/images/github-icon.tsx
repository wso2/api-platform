...
import { useMounted } from '@oxygen-ui/react/src/hooks/useMounted';
import { ReactNode } from 'react';

const GitHubIcon = (props: React.SVGProps<SVGSVGElement>): ReactNode => {
  const { mode, systemMode } = useColorScheme();

  const mounted: boolean = useMounted();

  if (!mounted) {
    // Render nothing or a placeholder to avoid hydration mismatch
    return null;
  }

  const resolvedMode = mode === 'system' ? systemMode : mode;
  const fillColor: string = resolvedMode === 'dark' ? '#FFF' : '#24292F';

  return (
    <svg focusable="false" aria-hidden="true" width="24" height="24" viewBox="0 0 24 24" {...props}>
      <path
        fill={fillColor}
