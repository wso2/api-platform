...
func main() {
	...
	// The shared outbound HTTP client must exist before server wiring that reaches it.
	if err := server.InitSharedHTTPClient(cfg); err != nil {
		slogger.Error("Failed to initialize shared outbound HTTP client", "error", err)
		os.Exit(1)
	}
