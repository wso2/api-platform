...
// The single configured APIP_CP_ENCRYPTION_KEY is used for all at-rest encryption.
func getSubscriptionTokenEncryptionKey() ([]byte, error) {
	cfg := config.GetConfig()
	if cfg.EncryptionKey == "" {
		return nil, fmt.Errorf("subscription token encryption requires APIP_CP_ENCRYPTION_KEY")
	}
...
// With the single consolidated APIP_CP_ENCRYPTION_KEY there is at most one candidate; the slice
