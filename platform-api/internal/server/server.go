...
func buildAuthenticator(cfg *config.Server, slogger *slog.Logger, roleScopeMap map[string][]string) (middleware.Authenticator, error) {
	if cfg.Auth.Mode != config.AuthModeIDP {
		...
		// caller and forwards an unsigned internal token (see JWT.SkipValidation).
		// No public key is needed or loaded; claims are still read from the token.
		if cfg.Auth.JWT.SkipValidation {
			slogger.Warn("Auth mode: jwt with signature validation DISABLED (auth.jwt.skip_validation=true) — accept unsigned tokens ONLY behind a trusted mediation layer on a private network")
