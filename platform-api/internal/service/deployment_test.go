...
// TestEffectiveOverrideDocument covers what a new deployment records as its
// override document: a promoted deployment inherits the base's, a request
// document is deep-merged on top of it, and nothing is stored when neither
// carries one.
func TestEffectiveOverrideDocument(t *testing.T) {
	baseWith := func(overrides interface{}) *model.Deployment {
		return &model.Deployment{Metadata: map[string]any{constants.MetadataKeyOverrides: overrides}}
	}

	t.Run("nil when neither base nor request has overrides", func(t *testing.T) {
		if got := effectiveOverrideDocument(nil, nil); got != nil {
			t.Errorf("expected nil, got %v", got)
		}
		if got := effectiveOverrideDocument(&model.Deployment{}, nil); got != nil {
			t.Errorf("expected nil for base without metadata, got %v", got)
		}
	})

	t.Run("request document is used when there is no base", func(t *testing.T) {
		requested := map[string]interface{}{"spec": map[string]interface{}{"cors": true}}
		got := effectiveOverrideDocument(nil, &requested)
		spec, _ := asStringKeyedMap(got["spec"])
		if spec["cors"] != true {
			t.Errorf("request override not recorded: %v", got)
		}
	})

	t.Run("promotion inherits the base document", func(t *testing.T) {
		base := baseWith(map[string]interface{}{
			"spec": map[string]interface{}{"cors": true, "timeout": 30},
		})
		got := effectiveOverrideDocument(base, nil)
		spec, _ := asStringKeyedMap(got["spec"])
		if spec["cors"] != true || spec["timeout"] != 30 {
			t.Errorf("base override not inherited: %v", got)
		}
	})

	t.Run("request is deep-merged over the inherited document", func(t *testing.T) {
		base := baseWith(map[string]interface{}{
			"spec": map[string]interface{}{"cors": true, "timeout": 30},
		})
		requested := map[string]interface{}{
			"spec": map[string]interface{}{"timeout": 60, "retries": 3},
		}
		got := effectiveOverrideDocument(base, &requested)
		spec, _ := asStringKeyedMap(got["spec"])
		if spec["cors"] != true {
			t.Errorf("inherited-only field lost: %v", spec)
		}
		if spec["timeout"] != 60 {
			t.Errorf("request should replace the inherited value: %v", spec)
		}
		if spec["retries"] != 3 {
			t.Errorf("request-only field missing: %v", spec)
		}
	})

	t.Run("a base whose overrides are not a map is ignored", func(t *testing.T) {
		if got := effectiveOverrideDocument(baseWith("not-a-map"), nil); got != nil {
			t.Errorf("expected nil, got %v", got)
		}
	})
}
