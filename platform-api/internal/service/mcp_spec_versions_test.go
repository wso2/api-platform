...
func apiSpecVersions(vs ...string) *[]api.MCPProxyMcpSpecVersions {
	out := make([]api.MCPProxyMcpSpecVersions, 0, len(vs))
	for _, v := range vs {
...
func apiSpecVersion(v string) *api.MCPProxyMcpSpecVersion {
	sv := api.MCPProxyMcpSpecVersion(v)
	return &sv
}
