...
func TestAttachProjectHandleUsesProjectLookup(t *testing.T) {
	apiModel := &model.API{
		ProjectID:      "019feb20-aaaa-bbbb-cccc-ddddeeeeffff",
		OrganizationID: "org-1",
	}

	attachProjectHandle(&mockProjectRepository{
		projectByUUID: &model.Project{ID: apiModel.ProjectID, Handle: "new-project"},
	}, apiModel, nil)
	if apiModel.ProjectHandle != "new-project" {
		t.Fatalf("ProjectHandle = %q, want %q", apiModel.ProjectHandle, "new-project")
	}

	unset := &model.API{ProjectID: apiModel.ProjectID}
	attachProjectHandle(nil, unset, nil)
	if unset.ProjectHandle != "" {
		t.Fatalf("ProjectHandle = %q, want empty when project repo is absent", unset.ProjectHandle)
	}
}

func TestAttachProjectHandleSoftFailsOnLookupError(t *testing.T) {
	apiModel := &model.API{
		ProjectID:      "019feb20-aaaa-bbbb-cccc-ddddeeeeffff",
		OrganizationID: "org-1",
	}
	...
	attachProjectHandle(&mockProjectRepository{
		projectByUUIDErr: errors.New("db unavailable"),
	}, apiModel, nil)
	if apiModel.ProjectHandle != "" {
		t.Fatalf("ProjectHandle = %q, want empty when project lookup fails", apiModel.ProjectHandle)
	}
}
