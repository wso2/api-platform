...
// TestPreserveAttachmentCredentialsWillNotCarryASecretToAnotherVendor: a routing
// name can outlive the provider behind it. Carrying the credential across on the
// strength of the name alone would send one vendor's secret to another's
// upstream on the next deployment.
func TestPreserveAttachmentCredentialsWillNotCarryASecretToAnotherVendor(t *testing.T) {
	existing := []model.LLMProxyAttachment{{
		ID: "openai-provider", Alias: "chat", IsPrimary: true,
		Auth: &model.UpstreamAuth{Type: "api-key", Header: "X-API-Key", Value: "{{ secret \"openai\" }}"},
	}}
	// Same name, different provider, and the redacted auth a client sends back
	// when it is not changing the credential.
	updated := []model.LLMProxyAttachment{{
		ID: "anthropic-provider", Alias: "chat", IsPrimary: true,
		Auth: &model.UpstreamAuth{Type: "api-key", Header: "X-API-Key"},
	}}

	got := preserveAttachmentCredentials(existing, updated)

	if value := upstreamAuthValue(got[0].Auth); value != "" {
		t.Fatalf("carried %q onto a different provider, want nothing carried", value)
	}
}
