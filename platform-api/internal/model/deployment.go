...
type Deployment struct {
	...
	// BuildID is the readable id of the build BuildUUID points at, joined from the
	// builds table rather than stored here: one place records the origin, and the
	// name for it is always read back through that, so the two cannot disagree. Nil
	// whenever BuildUUID is.
...
type MCPProxyDeploymentSpec struct {
	...
	SpecVersions []string         `yaml:"specVersions,omitempty"`
