...
// the values before a MCPProxyConfiguration exists. singleVersion is the deprecated
// single-valued form, read only when the list declares nothing.
func FoldSpecVersions(versions []string, singleVersion string) []string {
	if len(versions) > 0 {
		return versions
	}
	if singleVersion != "" {
