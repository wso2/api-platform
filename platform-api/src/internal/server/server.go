...
func StartPlatformAPIServer(cfg *config.Server, slogger *slog.Logger) (*Server, error) {
	...
	internalGatewayService := service.NewGatewayInternalAPIService(apiRepo, subscriptionRepo, llmProviderRepo, llmProxyRepo, deploymentRepo, gatewayRepo, orgRepo, projectRepo, cfg, slogger)
