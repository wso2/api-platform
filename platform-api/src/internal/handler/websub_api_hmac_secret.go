...
func (h *WebSubAPIHmacSecretHandler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /api/v1/websub-apis/{apiHandle}/secrets", h.CreateHmacSecret)
	mux.HandleFunc("GET /api/v1/websub-apis/{apiHandle}/secrets", h.ListHmacSecrets)
	mux.HandleFunc("DELETE /api/v1/websub-apis/{apiHandle}/secrets/{secretName}", h.DeleteHmacSecret)
	mux.HandleFunc("POST /api/v1/websub-apis/{apiHandle}/secrets/{secretName}/regenerate", h.RegenerateHmacSecret)
}
