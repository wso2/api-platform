...
func (h *GatewayHandler) CreateGateway(w http.ResponseWriter, r *http.Request) {
	...
	gateway, err := h.gatewayService.RegisterGateway(orgId, req.Handle, req.Name, description, req.Vhost,
...
func (h *GatewayHandler) GetGateway(w http.ResponseWriter, r *http.Request) {
	...
	gatewayId := r.PathValue("gatewayHandle")
	if gatewayId == "" {
		httputil.WriteJSON(w, http.StatusBadRequest, utils.NewErrorResponse(400, "Bad Request",
...
func (h *GatewayHandler) UpdateGateway(w http.ResponseWriter, r *http.Request) {
	...
	response, err := h.gatewayService.UpdateGateway(gatewayId, orgId, updatedBy, req.Description, req.Name, req.IsCritical, req.Properties)
	if err != nil {
...
func (h *GatewayHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET "+constants.APIBasePath+"/gateways/{gatewayHandle}", h.GetGateway)
	mux.HandleFunc("PUT "+constants.APIBasePath+"/gateways/{gatewayHandle}", h.UpdateGateway)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/gateways/{gatewayHandle}", h.DeleteGateway)
	mux.HandleFunc("GET "+constants.APIBasePath+"/gateways/{gatewayHandle}/tokens", h.ListTokens)
	mux.HandleFunc("POST "+constants.APIBasePath+"/gateways/{gatewayHandle}/tokens", h.RotateToken)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/gateways/{gatewayHandle}/tokens/{tokenId}", h.RevokeToken)
	mux.HandleFunc("GET "+constants.APIBasePath+"/gateways/{gatewayHandle}/live-proxy-artifacts", h.GetGatewayArtifacts)
	mux.HandleFunc("GET "+constants.APIBasePath+"/gateways/{gatewayHandle}/manifest", h.GetGatewayManifest)
