...
func (h *LLMProxyAPIKeyHandler) ListAPIKeys(w http.ResponseWriter, r *http.Request) {
	...
	proxyID := r.PathValue("proxyHandle")
	if proxyID == "" {
...
func (h *LLMProxyAPIKeyHandler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST "+constants.APIBasePath+"/llm-proxies/{proxyHandle}/api-keys", h.CreateAPIKey)
	mux.HandleFunc("GET "+constants.APIBasePath+"/llm-proxies/{proxyHandle}/api-keys", h.ListAPIKeys)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/llm-proxies/{proxyHandle}/api-keys/{keyName}", h.DeleteAPIKey)
}
