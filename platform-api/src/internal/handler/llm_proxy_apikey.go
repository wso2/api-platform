...
func (h *LLMProxyAPIKeyHandler) ListAPIKeys(c *gin.Context) {
	...
	response, err := h.apiKeyService.ListLLMProxyAPIKeys(c.Request.Context(), proxyID, orgID)
	if err != nil {
...
func (h *LLMProxyAPIKeyHandler) DeleteAPIKey(c *gin.Context) {
	...
	err := h.apiKeyService.DeleteLLMProxyAPIKey(c.Request.Context(), proxyID, orgID, userID, keyName)
	if err != nil {
		if errors.Is(err, constants.ErrAPINotFound) {
			c.JSON(http.StatusNotFound, utils.NewErrorResponse(404, "Not Found",
				"LLM proxy not found"))
