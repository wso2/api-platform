...
func (h *WebSubAPIKeyHandler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST "+constants.APIBasePath+"/websub-apis/{apiHandle}/api-keys", h.CreateAPIKey)
	mux.HandleFunc("PUT "+constants.APIBasePath+"/websub-apis/{apiHandle}/api-keys/{keyName}", h.UpdateAPIKey)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/websub-apis/{apiHandle}/api-keys/{keyName}", h.DeleteAPIKey)
}
