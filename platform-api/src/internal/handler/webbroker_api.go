...
func (h *WebBrokerAPIHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET "+constants.APIBasePath+"/webbroker-apis/{apiHandle}", h.GetWebBrokerAPI)
	mux.HandleFunc("PUT "+constants.APIBasePath+"/webbroker-apis/{apiHandle}", h.UpdateWebBrokerAPI)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/webbroker-apis/{apiHandle}", h.DeleteWebBrokerAPI)
}
