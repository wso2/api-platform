...
func (h *WebBrokerAPIDeploymentHandler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments", h.DeployWebBrokerAPI)
	mux.HandleFunc("POST "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments/{deploymentId}/undeploy", h.UndeployDeployment)
	mux.HandleFunc("POST "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments/{deploymentId}/restore", h.RestoreDeployment)
	mux.HandleFunc("GET "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments", h.GetDeployments)
	mux.HandleFunc("GET "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments/{deploymentId}", h.GetDeployment)
	mux.HandleFunc("DELETE "+constants.APIBasePath+"/webbroker-apis/{apiHandle}/deployments/{deploymentId}", h.DeleteDeployment)
}
...
func (h *WebBrokerAPIDeploymentHandler) DeployWebBrokerAPI(w http.ResponseWriter, r *http.Request) {
	...
	if req.GatewayHandle == "" {
		httputil.WriteJSON(w, http.StatusBadRequest, utils.NewErrorResponse(400, "Bad Request", "gatewayHandle is required"))
		return
	}
