...
func TestCascade_DeleteWebSubAPIRemovesHmacSecrets(t *testing.T) {
	...
	it.execT(t, `INSERT INTO organizations (uuid, handle, display_name, region, idp_organization_ref_uuid) VALUES (?, ?, ?, ?, ?)`,
		orgUUID, "wsc-"+orgUUID[:8], "cascade org", "us", "idp-ref")
	it.execT(t, `INSERT INTO projects (uuid, handle, display_name, organization_uuid) VALUES (?, ?, ?, ?)`,
		projectUUID, "cascade-proj", "cascade-proj", orgUUID)
	it.execT(t, `INSERT INTO artifacts (uuid, type, organization_uuid) VALUES (?, ?, ?)`,
		artifactUUID, "WebSubApi", orgUUID)
	it.execT(t, `INSERT INTO websub_apis (uuid, organization_uuid, handle, display_name, version, project_uuid, lifecycle_status, configuration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		artifactUUID, orgUUID, "ws-api-"+artifactUUID[:8], "ws-api", "v1.0", projectUUID, "CREATED", []byte("{}"))

	secret1 := id()
	secret2 := id()
	it.execT(t, `INSERT INTO websub_api_hmac_secrets (uuid, artifact_uuid, handle, encrypted_secret, status) VALUES (?, ?, ?, ?, ?)`,
		secret1, artifactUUID, "github-secret", []byte("enc1"), "active")
	it.execT(t, `INSERT INTO websub_api_hmac_secrets (uuid, artifact_uuid, handle, encrypted_secret, status) VALUES (?, ?, ?, ?, ?)`,
		secret2, artifactUUID, "gitlab-secret", []byte("enc2"), "active")

	if got := it.countT(t, "websub_api_hmac_secrets", "artifact_uuid", artifactUUID); got != 2 {
		t.Fatalf("precondition: want 2 hmac secrets, got %d", got)
	}
	if got := it.countT(t, "websub_apis", "uuid", artifactUUID); got != 1 {
		t.Fatalf("precondition: want 1 websub_api row, got %d", got)
	}

	it.execT(t, `DELETE FROM artifacts WHERE uuid = ?`, artifactUUID)

	if got := it.countT(t, "websub_api_hmac_secrets", "artifact_uuid", artifactUUID); got != 0 {
		t.Fatalf("[%s] hmac secrets not cascade-deleted after artifact delete: %d remain", it.driver, got)
	}
	if got := it.countT(t, "websub_apis", "uuid", artifactUUID); got != 0 {
