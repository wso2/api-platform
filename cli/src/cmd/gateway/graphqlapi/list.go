...
const (
	ListCmdLiteral = "list"
	ListCmdExample = `# List all GraphQL APIs
ap gateway graphql-api list`
)

var listCmd = &cobra.Command{
	Use:     ListCmdLiteral,
	Short:   "List all GraphQL APIs on the gateway",
	Long:    "Retrieves and displays all GraphQL APIs deployed on the currently active gateway.",
...
// APIListItem is a list-view projection of a GraphQLAPI. The management API list
...
// APIListResponse represents the response from GET /graphql-apis
type APIListResponse struct {
	Status      string        `json:"status"`
	Count       int           `json:"count"`
	GraphQLAPIs []APIListItem `json:"graphqlApis"`
}
...
func runListCommand(cmd *cobra.Command) error {
	...
	// Call the /graphql-apis endpoint
	resp, err := client.Get(utils.GatewayGraphQLAPIsPath)
	if err != nil {
		return fmt.Errorf("failed to call %s endpoint: %w", utils.GatewayGraphQLAPIsPath, err)
	}
	...
	if resp.StatusCode == http.StatusNotFound {
		fmt.Println("No GraphQL APIs found on the gateway.")
		return nil
	}
	...
	rows := make([][]string, 0, len(listResp.GraphQLAPIs))
	for _, api := range listResp.GraphQLAPIs {
