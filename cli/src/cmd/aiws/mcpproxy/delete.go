...
var deleteCmd = &cobra.Command{
	...
	Short:   "Delete an MCP proxy from the AI workspace",
	Long:    "Delete an MCP proxy from the WSO2 API Platform AI workspace by its identifier.",
...
func init() {
	utils.AddStringFlag(deleteCmd, utils.FlagID, &deleteID, "", "MCP proxy ID (required)")
...
func runDeleteCommand() error {
	...
	if id == "" {
		return fmt.Errorf("MCP proxy ID is required")
	}
	...
	resp, err := client.Delete(aiworkspace.MCPProxyByIDPath(id))
	if err != nil {
		return aiworkspace.WrapRequestError("delete mcp proxy", err, deleteInsecure)
	}
	resp.Body.Close()

	fmt.Printf("MCP proxy %q deleted from ai-workspace %s (platform: %s)\n", id, aiWorkspace.Name, resolvedPlatform)
	return nil
}
