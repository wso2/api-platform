...
var listCmd = &cobra.Command{
	Use:     ListCmdLiteral,
	Short:   "List all MCP proxies in a project",
	Long:    "Retrieves all MCP proxies for a given project from the WSO2 API Platform AI workspace.",
...
func init() {
	utils.AddStringFlag(listCmd, utils.FlagProjectID, &listProjectID, "", "Project ID (required)")
	utils.AddStringFlag(listCmd, utils.FlagLimit, &listLimit, "", "Maximum number of proxies to return")
	utils.AddStringFlag(listCmd, utils.FlagOffset, &listOffset, "", "Number of proxies to skip")
...
func runListCommand() error {
	...
	projectID := strings.TrimSpace(listProjectID)
	if projectID == "" {
		return fmt.Errorf("project ID is required (use --project-id)")
	}
	...
	path := aiworkspace.MCPProxyListPath(projectID, aiworkspace.ListQuery{Limit: listLimit, Offset: listOffset})

	resp, err := client.Get(path)
	if err != nil {
		return aiworkspace.WrapRequestError("list mcp proxies", err, listInsecure)
	}
