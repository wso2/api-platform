...
var getCmd = &cobra.Command{
	Use:     GetCmdLiteral,
	Short:   "Get one or all LLM proxies from the AI workspace",
	Long:    "Retrieve LLM proxies from the WSO2 API Platform AI workspace. With --id a single proxy is fetched by its identifier; without it all proxies in the project are listed (--project-id is required for listing).",
...
func init() {
	utils.AddStringFlag(getCmd, utils.FlagID, &getID, "", "LLM proxy ID (omit to list all)")
...
func runGetCommand() error {
	...
	if id := strings.TrimSpace(getID); id != "" {
		...
		path = aiworkspace.ProxyByIDPath(id)
		action = "get llm proxy"
	} else {
		...
		path = aiworkspace.ProxyListPath(projectID, aiworkspace.ListQuery{Limit: getLimit, Offset: getOffset})
