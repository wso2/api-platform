...
var deleteCmd = &cobra.Command{
	...
	Short:   "Delete an LLM proxy from the AI workspace",
	Long:    "Delete an LLM proxy from the WSO2 API Platform AI workspace by its identifier.",
...
func init() {
	utils.AddStringFlag(deleteCmd, utils.FlagID, &deleteID, "", "LLM proxy ID (required)")
...
func runDeleteCommand() error {
	...
	if id == "" {
		return fmt.Errorf("LLM proxy ID is required")
	}
	...
	resp, err := client.Delete(aiworkspace.ProxyByIDPath(id))
	if err != nil {
		return aiworkspace.WrapRequestError("delete llm proxy", err, deleteInsecure)
	}
	resp.Body.Close()

	fmt.Printf("LLM proxy %q deleted from ai-workspace %s (platform: %s)\n", id, aiWorkspace.Name, resolvedPlatform)
	return nil
}
