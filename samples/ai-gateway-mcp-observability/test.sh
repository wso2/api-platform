...
# Prometheus is actually scraping them, the metrics carry the per-proxy label the
# dashboard groups by, the MCP policies record the calls they stopped, Grafana has
# the dashboard, and Jaeger has traces.
#
...
# The api_name label carries spec.displayName from mcp-proxy-toolbox.yaml and
# mcp-proxy-metered.yaml, not metadata.name.
EXPECTED_PROXIES=("Toolbox MCP" "Metered MCP")

# Matches both MCP proxy routes. The route label contains the request path, so this
# is the same filter the dashboard panels use.
MCP_ROUTE_RE='.*/(toolbox|metered)/mcp.*'
...
# Runs one instant query and prints the raw Prometheus response.
promql() {
  curl -s --connect-timeout 5 --max-time 15 --get "${PROMETHEUS_URL}/api/v1/query" \
...
#
# Only the request_headers phase records api_name; the later phases report empty
# strings. Every per-proxy panel therefore filters on that phase.
# ---------------------------------------------------------------------------
echo ""
echo "══════════════════════════════════════════════════"
...
RESULT=$(promql "sum by (api_name) (policy_engine_requests_total{phase=\"request_headers\", route=~\"${MCP_ROUTE_RE}\"})")
...
if ! jq -e '.status == "success"' >/dev/null 2>&1 <<< "${RESULT}"; then
  ...
  # Both proxies must report. One answering while the other is silent is a failure.
  MISSING=""
...
# ---------------------------------------------------------------------------
# Test 4: the MCP policies recorded the calls they stopped
#
# A tool name never appears in the URL, so the policy_name label is what tells an
# allowlist rejection apart from a rate limit.
# ---------------------------------------------------------------------------
echo ""
echo "══════════════════════════════════════════════════"
echo " Test 4: MCP policy rejections are recorded"
echo "══════════════════════════════════════════════════"

# This metric is not labelled by route, so the query covers the whole gateway. Only
# the two MCP proxies are deployed here, so every rejection counted is an MCP call.
RESULT=$(promql "sum by (policy_name) (policy_engine_short_circuits_total)")

if ! jq -e '.status == "success"' >/dev/null 2>&1 <<< "${RESULT}"; then
  fail "Prometheus query failed for policy rejections."
else
  REJECTIONS=$(jq -r '.data.result[] | "\(.metric.policy_name)=\(.value[1])"' <<< "${RESULT}")
  if [[ -z "${REJECTIONS}" ]]; then
    fail "No policy rejections recorded. Run ./load.sh — it sends calls without a
        token and calls for a tool that is not on the allowlist, and both should
        land here."
  else
    pass "Policy rejections recorded:"
    while IFS= read -r line; do printf '        %s\n' "${line}"; done <<< "${REJECTIONS}"
  fi
fi

# ---------------------------------------------------------------------------
# Test 5: Grafana provisioned the dashboard
# ---------------------------------------------------------------------------
echo ""
echo "══════════════════════════════════════════════════"
echo " Test 5: Grafana dashboard is provisioned"
echo "══════════════════════════════════════════════════"

SEARCH=$(curl -s --connect-timeout 5 --max-time 15 -u "${GRAFANA_AUTH}" \
  "${GRAFANA_URL}/api/search?query=AI%20Gateway%20MCP" 2>/dev/null)
...
# ---------------------------------------------------------------------------
# Test 6: traces reached Jaeger
# ---------------------------------------------------------------------------
echo ""
echo "══════════════════════════════════════════════════"
echo " Test 6: Traces are visible in Jaeger"
echo "══════════════════════════════════════════════════"
