#!/usr/bin/env bash
#
# Generates about a minute of mixed MCP traffic through both proxies so the
# dashboard has something real to show: steady successes, tool failures, calls
# refused by the allowlist, calls rejected for having no token, and — once the
# metered proxy has spent its per-tool allowance — 429s.
#
...
#
# First, a tool call is never a single request. The client introduces itself with
# "initialize", the server answers with a session id in the mcp-session-id
# response header, and every later call sends that id back.
#
# Second, the proxies require an access token, and the subject inside the token
# identifies the caller. So this script opens three sessions, each with its own
# client name and its own token subject.
#
set -uo pipefail
...
# Runs the handshake and prints the session id. Retries, since a proxy may need a
# moment before it starts serving.
open_session() {
  local sid attempt
...
send() {
  ...
  # Replies arrive as server-sent events, so the JSON payload is the part of the
  # body that follows "data: ".
...
info "Metered : ${METERED_URL}   (15 calls per tool per minute — expect 429s part-way through)"
echo ""
...
while [[ $(date +%s) -lt ${end} ]]; do
  ...
  case $(( i % 12 )) in
    ...
    # Valid tool, invalid argument. The server runs it and reports a failure, so
    # the call is an HTTP 200 whose JSON-RPC body carries the error.
...
echo ""
echo "══════════════════════════════════════════════════"
echo " Done — ${total} requests across 3 sessions"
echo "══════════════════════════════════════════════════"
printf '  succeeded                 %5d\n' "${count_ok}"
printf '  tool execution errors     %5d   valid tool, invalid argument, HTTP 200\n' "${count_tool_error}"
printf '  refused by the allowlist  %5d   get-env\n' "${count_blocked}"
