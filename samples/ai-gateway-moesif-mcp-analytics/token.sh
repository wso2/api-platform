...
#
# The subject becomes the consumer name in analytics, so different subjects
# show up as different consumers.
#
# A real deployment would get tokens from an identity provider. The sample
# signs its own so it needs no extra service.

TOKEN_ISSUER="${TOKEN_ISSUER:-https://mcp-analytics-sample.local}"
TOKEN_AUDIENCE="${TOKEN_AUDIENCE:-mcp-analytics-sample}"
