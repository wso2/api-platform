...
#   ./teardown.sh --clean    also remove the extracted distribution
#
set -euo pipefail
...
for provider_yaml in "${PROVIDER_YAMLS[@]}"; do
  [[ -f "${provider_yaml}" ]] || error "Not found: ${provider_yaml}"
...
if docker ps -aq -f "name=^${MOCK_CONTAINER}$" 2>/dev/null | grep -q .; then
  ...
  if docker rm -f "${MOCK_CONTAINER}" >/dev/null 2>&1; then
    ...
    warn "Could not remove ${MOCK_CONTAINER}."
    failures=$(( failures + 1 ))
  fi
else
  info "${MOCK_CONTAINER} is not running."
fi
...
if (( failures == 0 )); then
  ...
  if [[ "${CLEAN}" == false ]]; then
    echo " Run with --clean to also remove ${DIST_NAME}/ and ${DIST_ZIP},"
    echo " so the next ./setup.sh starts from the unmodified distribution."
  fi
  echo "============================================================"
else
