...
| 2 | `Gateway` | Triggers operator: Helm install `{metadata.name}-gateway`, register controller Service in memory. |
...
kubectl apply -f 02a-gateway-values-configmap.yaml
...
7. **Invoke the routed API with `curl`** (through gateway-runtime Service):
   ...
   ```bash
   kubectl run curl-hello --rm -it --restart=Never --image=curlimages/curl -n gateway-api-demo -- \
     curl -skS -H 'Host: demo.gateway-api.local' \
     "https://platform-gw-gateway-gateway-runtime.gateway-api-demo.svc.cluster.local:8443/hello"
   ```
...
- `04-httproute.yaml` — `PathPrefix /hello`, GET → backend Service
