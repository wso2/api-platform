...
5. **No deferring a violation behind a code comment.** Never resolve an unescaped-interpolation or missing-sanitization finding by adding a `// TODO`/`FIXME`-style comment and shipping the template anyway — a comment does not encode output. Fix it before merging, or raise the gap explicitly for an approved exception rather than leaving it annotated in the source.
