...
8. **No deferring a violation behind a code comment.** Never resolve a path-traversal, zip-slip, unbounded-read, or content-type-allowlist gap by adding a `// TODO`/`FIXME`-style comment and shipping the code anyway — a comment is not a containment check and never satisfies this rule. Fix it before merging, or raise the gap explicitly for an approved exception rather than leaving it annotated in the source.
