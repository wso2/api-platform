...
2. **Approved algorithm selection:**
   ...
   |---|---|---|---|
   | Key Encapsulation | FIPS 203 | ML-KEM-768 (Kyber-768) | `github.com/cloudflare/circl/kem/kyber/kyber768` |
...
5. **Randomness and nonce safety.** Key generation must use `crypto/rand` exclusively — never `math/rand`, `time.Now().UnixNano()`, or a seeded PRNG. AES-GCM nonces (96-bit) must be freshly generated per encryption via `crypto/rand.Read` and never reused under the same key; rotate the key after 2³² encryptions. CIRCL's ML-KEM `Encapsulate` draws its own randomness from `crypto/rand` internally — don't pass external randomness unless the API requires it.
...
func Encapsulate(recipientClassical *ecdh.PublicKey, recipientPQC *kyber768.PublicKey) (HybridKEMCiphertext, []byte, error) {
    ...
    pqcCT, pqcShared, err := kyber768.Encapsulate(recipientPQC) // FIPS 203
    if err != nil {
        return HybridKEMCiphertext{}, nil, fmt.Errorf("ml-kem encapsulate: %w", err)
    }
