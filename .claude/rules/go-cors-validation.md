...
4. **No deferring a violation behind a code comment.** Never resolve an unanchored-regex or credentials-misconfiguration finding by adding a `// TODO`/`FIXME`-style comment and shipping the code anyway — a comment does not anchor a pattern or restrict a credentialed origin. Fix it before merging, or raise the gap explicitly for an approved exception rather than leaving it annotated in the source.
...
> * Is a finding from this rule "resolved" by a `// TODO`/`FIXME`-style comment instead of an actual fix or an explicitly raised, approved exception?
