...
func NewConsumer(brokers []string, groupID string, topics []string, handler connectors.MessageHandler) (*Consumer, error) {
	return NewConsumerWithConfig(ConnectionConfig{Brokers: brokers}, groupID, topics, handler)
}

// NewConsumerWithConfig creates a new shared-group Kafka consumer.
func NewConsumerWithConfig(cfg ConnectionConfig, groupID string, topics []string, handler connectors.MessageHandler) (*Consumer, error) {
...
func NewManualCommitConsumer(brokers []string, groupID string, topics []string, handler connectors.MessageHandler) (*ManualCommitConsumer, error) {
	return NewManualCommitConsumerWithConfig(ConnectionConfig{Brokers: brokers}, groupID, topics, handler)
}

// NewManualCommitConsumerWithConfig creates a consumer with manual offset commit.
func NewManualCommitConsumerWithConfig(cfg ConnectionConfig, groupID string, topics []string, handler connectors.MessageHandler) (*ManualCommitConsumer, error) {
	opts, err := BuildClientOptions(
		cfg,
		kgo.ConsumerGroup(groupID),
		kgo.ConsumeTopics(topics...),
