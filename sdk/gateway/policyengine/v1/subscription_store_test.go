...
func TestSubscriptionStore_ReplaceAllAndIsActive(t *testing.T) {
	...
	if len(store.appData) != 2 {
		t.Fatalf("expected 2 APIs in store, got %d", len(store.appData))
	}
	if len(store.appData["api-1"]) != 2 {
		t.Fatalf("expected 2 subscriptions for api-1, got %d", len(store.appData["api-1"]))
	}
	if len(store.appData["api-2"]) != 1 {
		t.Fatalf("expected 1 subscription for api-2, got %d", len(store.appData["api-2"]))
	}
