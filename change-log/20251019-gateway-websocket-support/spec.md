...
**Feature Branch**: `002-gateway-websockets-i`
**Created**: 2025-10-15
**Status**: Draft
**Input**: User description: "gateway-websockets I need a websocket service in the platform-api that will be used by the gateway to connect to the platform api. that will create a persistent connection. Once the connection is made it should be kept in memory associated to the particular gateway. Later we can use this connection to send events to the gateway. When Deploy API happens (you can check /home/malintha/wso2apim/gitworkspace/api-platform/platform-api/spec/impls/api-lifecycle-management.md) for details I need to select the particular gateway's connection and send a deployment event to that. We should write this eventing framework in an abstract manner sho that any kind of event sending can be easily pluggable. Also using websocket might also change in the future. That part should be also pluggable using an interface."
