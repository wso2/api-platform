...
// =============================================================================
// deepCopyParams failure: span status across all six chain entry points.
// =============================================================================

func badParamsSpec(name string) policy.PolicySpec {
	return policy.PolicySpec{
		Name:       name,
		Version:    "v1.0.0",
		Enabled:    true,
		Parameters: policy.PolicyParameters{Raw: map[string]interface{}{"bad": make(chan int)}},
	}
}

func TestParameterCloneFailure_SpanStatus_AllPhases(t *testing.T) {
	t.Run("request header", func(t *testing.T) {
		...
		_, err := executor.ExecuteRequestHeaderPolicies(context.Background(), []policy.Policy{pol}, reqCtx,
			[]policy.PolicySpec{badParamsSpec("hdr")}, "api", "route", false)

		require.Error(t, err)
		assert.Zero(t, pol.calls, "OnRequestHeaders must not run once param cloning fails")
		span := findSpanByName(sr.Ended(), "policy.request.hdr")
		require.NotNil(t, span)
		assertParameterCloneFailure(t, span)
	})

	t.Run("request body", func(t *testing.T) {
		executor, sr := newRecordedChainExecutor(t, nil)
		pol := &countingRequestBodyPolicy{mode: policy.ProcessingMode{RequestBodyMode: policy.BodyModeBuffer}}
		reqCtx := testutils.NewTestRequestContext()

		_, err := executor.ExecuteRequestPolicies(context.Background(), []policy.Policy{pol}, reqCtx,
			[]policy.PolicySpec{badParamsSpec("body")}, "api", "route", false)
	...
	t.Run("response header", func(t *testing.T) {
		...
		_, err := executor.ExecuteResponseHeaderPolicies(context.Background(), []policy.Policy{pol}, respCtx,
			[]policy.PolicySpec{badParamsSpec("resphdr")}, "api", "route", false)
	...
	t.Run("response body", func(t *testing.T) {
		executor, sr := newRecordedChainExecutor(t, nil)
		pol := &countingResponseBodyPolicy{mode: policy.ProcessingMode{ResponseBodyMode: policy.BodyModeBuffer}}
		respCtx := testutils.NewTestResponseContext()

		_, err := executor.ExecuteResponsePolicies(context.Background(), []policy.Policy{pol}, respCtx,
			[]policy.PolicySpec{badParamsSpec("respbody")}, "api", "route", false)
	...
	t.Run("streaming request", func(t *testing.T) {
		...
		_, err := executor.ExecuteStreamingRequestPolicies(context.Background(), []policy.Policy{pol}, reqCtx, chunk,
			[]policy.PolicySpec{badParamsSpec("streq")}, "api", "route", false)
	...
	t.Run("streaming response", func(t *testing.T) {
		...
		_, err := executor.ExecuteStreamingResponsePolicies(context.Background(), []policy.Policy{pol}, respCtx, chunk,
			[]policy.PolicySpec{badParamsSpec("stresp")}, "api", "route", false)
...
func assertParameterCloneFailure(t *testing.T, span sdktrace.ReadOnlySpan) {
	t.Helper()
	status := span.Status()
	assert.Equal(t, codes.Error, status.Code)
	assert.Equal(t, "parameter clone failed", status.Description)
	require.Len(t, span.Events(), 1)
	assert.Equal(t, "exception", span.Events()[0].Name)
}
