...
# xDS server, on by default -- matching server.xds_tls.enabled's default of
# true in config-template.toml. Set XDS_TLS_ENABLED=false to fall back to
# plaintext xDS when talking to a gateway-controller build/config where
# server.xds_tls.enabled=false. This cert/key/CA material is entirely local
# to this container -- gateway-controller never needs to know these paths.
# SDS/secret delivery rides the same ADS stream this bootstrap xds_cluster
# already opens (see pkg/xds/translator.go's createUpstreamTLSContext, which
# references the SDS config source via ConfigSource_Ads rather than a
# second, TLS-duplicating cluster), so there is no separate controller-side
# copy of this TLS material to keep in sync.
export XDS_TLS_ENABLED="${XDS_TLS_ENABLED:-true}"
...
# policy_engine.xds.tls.enabled in config-template.toml -- both default to
# true, so an unset POLICY_ENGINE_XDS_TLS_ENABLED tracks XDS_TLS_ENABLED's
# own (also on-by-default) value rather than silently falling to plaintext.
POLICY_ENGINE_XDS_TLS_ENABLED_EFFECTIVE="${POLICY_ENGINE_XDS_TLS_ENABLED:-${XDS_TLS_ENABLED}}"
...
# is used when XDS_TLS_ENABLED=false, falling xds_cluster back to plaintext;
# otherwise (the default -- XDS_TLS_ENABLED defaults to true) a flow-style
# (JSON-like) UpstreamTlsContext mapping is used, kept on one line so
# config-override.yaml stays valid YAML both before and after envsubst.
# Cert/key/CA are referenced by filename, read directly from disk
