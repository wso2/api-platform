...
// Helper function to create properly formatted API key for client usage (apip_<hex random>.<hex encoded API key ID>)
func createAPIKeyWithID(plainAPIKey, id string) string {
	hexEncodedID := hex.EncodeToString([]byte(id))
	return plainAPIKey + "." + hexEncodedID
}
...
func TestAPIKeyPolicy_ValidKeyInHeader(t *testing.T) {
	...
	setupValidAPIKeyStore(t, "test-api-id", plainAPIKey)

	// Create request context with valid API key in header
	// Client receives: apip_<64hex>.<hex_id>
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
...
func TestAPIKeyPolicy_ValidKeyWithPrefix(t *testing.T) {
	...
	// Create request context with valid API key with Bearer prefix
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
	headers := map[string][]string{
...
func TestAPIKeyPolicy_CaseInsensitiveHeader(t *testing.T) {
	...
	// Use different casing for header name
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
	headers := map[string][]string{
		"x-api-key": {apiKeyValue},
	}
...
func TestAPIKeyPolicy_QueryParamWithSpecialCharacters(t *testing.T) {
	...
	// Setup mock API key store with valid key (simple key, test URL encoding mechanics)
	setupValidAPIKeyStore(t, "test-api-id", plainAPIKey)

	// Create request context with URL-encoded API key in query parameter
	// Test a simple case: spaces encoded as %20
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
...
func TestAPIKeyPolicy_MultipleHeaderValues(t *testing.T) {
	...
	// Set multiple values for the same header (should use first one)
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
	headers := map[string][]string{
...
func TestAPIKeyPolicy_LongAPIKey(t *testing.T) {
	...
	setupValidAPIKeyStore(t, "test-api-id", longPlainAPIKey)

	apiKeyValue := createAPIKeyWithID(longPlainAPIKey, "test-id")
	headers := map[string][]string{
		"X-API-Key": {apiKeyValue},
	}
	ctx := createMockRequestContext("/api/test", headers)
...
func TestAPIKeyPolicy_MultipleQueryParams(t *testing.T) {
	...
	// Create URL with multiple instances of the same query parameter
	apiKeyValue := createAPIKeyWithID(plainAPIKey, "test-id")
...
func setupValidAPIKeyStore(t *testing.T, apiId, plainAPIKey string) {
	...
	testAPIKey := &policy.APIKey{
		...
		APIId:      apiId,
		Operations: `["GET /test", "POST /test"]`, // JSON array of "METHOD path" strings
