...
func createTestClientWithConfig(t *testing.T, cfg config.ControlPlaneConfig) *Client {
	...
	return NewClient(cfg, logger, store, db, nil, nil, routerConfig, nil, nil, apiKeyConfig, nil, systemConfig, nil, nil, nil, nil, mockHub, nil, nil, nil, testHTTPClient(cfg.InsecureSkipVerify))
}
