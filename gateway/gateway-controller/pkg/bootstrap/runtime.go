...
// Package bootstrap provides the Run function that wires all gateway-controller
// components together and starts the server. Extracting it from package main
// allows alternative entry points (e.g. event-gateway-controller) to reuse
// the same startup sequence while injecting extension-specific components via
// the controllerext.ControllerExtension interface.
package bootstrap
