...
func TestValidateXDSServerTLS(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "blank allowed client identity is rejected",
			mutate:      func(c *XDSServerTLSConfig) { c.AllowedClientIdentities = []string{"   "} },
			wantErr:     true,
			errContains: "allowed_client_identities must not contain blank identities",
		},
		{
...
func TestBuildXDSServerTLSConfig_PQCHandshakeNegotiation(t *testing.T) {
	...
	dial := func(t *testing.T, clientCurves []tls.CurveID) tls.CurveID {
		...
		go func() {
			...
			if err := tlsConn.HandshakeContext(serverCtx); err != nil {
				errCh <- err
				return
			}
		...
		conn, err := dialer.DialContext(clientCtx, "tcp", ln.Addr().String())
		require.NoError(t, err)
		defer func() { _ = conn.Close() }()
