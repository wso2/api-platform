...
type XDSServerTLSConfig struct {
	...
	// most preferred first (e.g. "X25519,P-256"). A hybrid post-quantum
	// group ("X25519MLKEM768", FIPS 203 ML-KEM-768 + X25519) can be
	// prepended once both the Envoy/policy-engine peers reaching this
	// server are confirmed to support it -- this server is Go's own
	// crypto/tls (1.23+ implements X25519MLKEM768 natively), so an
	// unsupporting peer simply falls back to a later classical entry in
	// this same list.
