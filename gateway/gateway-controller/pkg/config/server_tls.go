...
func ParseServerEcdhCurves(raw string) ([]tls.CurveID, error) {
	...
	for _, part := range parts {
		...
		curve, ok := serverEcdhCurvesByName[name]
		if !ok {
...
// serverTLSVersionByName maps the version strings accepted in
// ServerTLSConfig.MinimumProtocolVersion/MaximumProtocolVersion to Go's
// crypto/tls version identifiers. Same vocabulary ("TLS1_2", etc.) as
// router.downstream_tls/upstream_tls for consistency within this shared
...
var serverTLSVersionByName = map[string]uint16{
...
// serverTLSVersionOrder ranks the version names above so a min > max
// combination can be rejected at validation time.
var serverTLSVersionOrder = map[string]int{
	"TLS1_0": 0,
	"TLS1_1": 1,
	"TLS1_2": 2,
	"TLS1_3": 3,
...
func ParseServerTLSVersion(name string) (version uint16, ok bool) {
	version, ok = serverTLSVersionByName[name]
	return version, ok
}

// serverCipherSuiteByName is built from Go's own list of secure cipher
// suites (tls.CipherSuites — deliberately excludes tls.InsecureCipherSuites)
// so an operator can only ever restrict to suites Go itself considers safe,
// never re-enable a weak one. Includes the three TLS 1.3 suite names for
...
var serverCipherSuiteByName = func() map[string]uint16 {
...
func ParseServerCiphers(raw string) ([]uint16, error) {
	...
	for _, part := range parts {
		...
		id, ok := serverCipherSuiteByName[name]
		if !ok {
