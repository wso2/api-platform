...
// carried through the extra hop, so the cluster name is clusterkey.APILevelName(env, UUID)
...
func TestLLMTransformer_ClusterNameKeyedOnLLMUUID(t *testing.T) {
	...
	assert.Equal(t, clusterkey.APILevelName("main", cfg.UUID), mainRoute.Upstream.ClusterKey,
