...
// ─── The bodyResolved injection ──────────────────────────────────────────────

// The parameter name is a silent contract: the controller writes this string and each policy
// reads it in its own repository, so a rename here does not fail to compile — it just leaves
// every resolver-aware policy buffering and parsing the body again.
func TestApplyResolverAwareParams_ParameterNameIsTheContract(t *testing.T) {
	assert.Equal(t, "bodyResolved", paramBodyResolved,
		"policies read this literal; changing it silently disables them")
}

// Analytics is prepended to every chain, so on a resolver-bearing route it must be told, or it
// parses the body a sixth time for facts the resolver already published.
func TestApplyResolverAwareParams_CoversTheAnalyticsCollector(t *testing.T) {
	assert.True(t, resolverAwarePolicies[constants.ANALYTICS_SYSTEM_POLICY_NAME])
}

func TestApplyResolverAwareParams_TellsEveryMemberAndNobodyElse(t *testing.T) {
	chain := []policyenginev1.PolicyInstance{
		{Name: "mcp-auth"},
		{Name: "mcp-authz", Parameters: map[string]interface{}{"existing": "kept"}},
		{Name: "mcp-ratelimit"},
		{Name: "mcp-acl-list"},
		{Name: constants.ANALYTICS_SYSTEM_POLICY_NAME},
		{Name: "mcp-rewrite"}, // mutates the body, so it can never decline it
		{Name: "jwt-auth"},    // not an MCP policy at all
	}

	applyResolverAwareParams(chain)

	for _, p := range chain {
		resolved, present := p.Parameters[paramBodyResolved]
		if resolverAwarePolicies[p.Name] {
			assert.True(t, present && resolved == true, "%s should have been told", p.Name)
		} else {
			assert.False(t, present, "%s is not resolver-aware and must be left alone", p.Name)
		}
	}

	assert.Equal(t, "kept", chain[1].Parameters["existing"],
		"injection must not discard parameters the policy already had")
}

// Only the multiplexed POST route resolves a body, so only its chain may be told. The same
// policies on GET or DELETE /mcp keep their default and buffer, which costs nothing there.
func TestMCPResolution_OnlyTheMultiplexedRouteIsTold(t *testing.T) {
	rdc := transformMCP(t, makeMCPStoredConfig(t, "2025-06-18"))
	...
	postChain := rdc.PolicyChains[rdc.ChainKeyFor(post.Vhost, mcpResolverOperation)]
	require.NotNil(t, postChain)
	for _, p := range rdc.PolicyChains[getKey].Policies {
		_, present := p.Params[paramBodyResolved]
		assert.False(t, present, "%s on GET /mcp must not be told the body is resolved", p.Name)
	}
}
