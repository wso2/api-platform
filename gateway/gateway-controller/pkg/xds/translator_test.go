...
// TestBuildRedirectAction guards the structured Gateway-API RequestRedirect rendering. Each Envoy
// RedirectAction component is set ONLY when the redirect specifies it; anything omitted is left
// unset so Envoy preserves it from the original request (scheme, host, port, path). This is the core
// correctness contract for scheme-only / host-only / port-only / status-only redirects. Unsupported
// status codes return nil so createRedirectRoute falls back to a direct_response.
func TestBuildRedirectAction(t *testing.T) {
	strptr := func(s string) *string { return &s }
	intptr := func(i int) *int { return &i }

	// Status-only (301): code set; scheme/host/port/path all preserved (unset).
	ra := buildRedirectAction(&models.RouteRedirect{StatusCode: 301})
	require.NotNil(t, ra)
	assert.Equal(t, route.RedirectAction_MOVED_PERMANENTLY, ra.GetResponseCode())
	assert.Empty(t, ra.GetHostRedirect(), "hostname omitted must preserve request authority")
	assert.Empty(t, ra.GetSchemeRedirect(), "scheme omitted must preserve request scheme")
	assert.Zero(t, ra.GetPortRedirect(), "port omitted must preserve request port")
	assert.Empty(t, ra.GetPathRedirect(), "path omitted must preserve request path")
	assert.Empty(t, ra.GetPrefixRewrite(), "path omitted must preserve request path")

	// Omitted statusCode (0) defaults to 302/FOUND.
	ra = buildRedirectAction(&models.RouteRedirect{})
	require.NotNil(t, ra)
	assert.Equal(t, route.RedirectAction_FOUND, ra.GetResponseCode())

	// Scheme-only: SchemeRedirect set, HostRedirect left unset (preserved).
	ra = buildRedirectAction(&models.RouteRedirect{StatusCode: 302, Scheme: strptr("https")})
	require.NotNil(t, ra)
	assert.Equal(t, "https", ra.GetSchemeRedirect())
	assert.Empty(t, ra.GetHostRedirect())

	// Hostname-only: HostRedirect set, scheme/port/path unset.
	ra = buildRedirectAction(&models.RouteRedirect{StatusCode: 302, Hostname: strptr("example.org")})
	require.NotNil(t, ra)
	assert.Equal(t, "example.org", ra.GetHostRedirect())
	assert.Empty(t, ra.GetSchemeRedirect())
	assert.Zero(t, ra.GetPortRedirect())

	// Port-only.
	ra = buildRedirectAction(&models.RouteRedirect{StatusCode: 302, Port: intptr(8443)})
	require.NotNil(t, ra)
	assert.Equal(t, uint32(8443), ra.GetPortRedirect())

	// Path ReplaceFullPath -> PathRedirect.
	ra = buildRedirectAction(&models.RouteRedirect{StatusCode: 302, Path: &models.RouteRedirectPath{
		Type: "ReplaceFullPath", ReplaceFullPath: strptr("/new"),
	}})
	require.NotNil(t, ra)
	assert.Equal(t, "/new", ra.GetPathRedirect())

	// Path ReplacePrefixMatch -> PrefixRewrite.
	ra = buildRedirectAction(&models.RouteRedirect{StatusCode: 302, Path: &models.RouteRedirectPath{
		Type: "ReplacePrefixMatch", ReplacePrefixMatch: strptr("/new-prefix"),
	}})
	require.NotNil(t, ra)
	assert.Equal(t, "/new-prefix", ra.GetPrefixRewrite())

	// Combined scheme+host+port+status.
	ra = buildRedirectAction(&models.RouteRedirect{
		StatusCode: 308, Scheme: strptr("https"), Hostname: strptr("h.example.org"), Port: intptr(443),
	})
	require.NotNil(t, ra)
	assert.Equal(t, route.RedirectAction_PERMANENT_REDIRECT, ra.GetResponseCode())
	assert.Equal(t, "https", ra.GetSchemeRedirect())
	assert.Equal(t, "h.example.org", ra.GetHostRedirect())
	assert.Equal(t, uint32(443), ra.GetPortRedirect())

	// Unsupported status code -> nil (caller falls back to direct_response).
	assert.Nil(t, buildRedirectAction(&models.RouteRedirect{StatusCode: 300}))
	// Nil redirect -> nil.
	assert.Nil(t, buildRedirectAction(nil))
}

// TestCreateRedirectRoute verifies the route-level wiring: a supported status yields a native
// Envoy RedirectAction with the request match populated (shared with normal routes), and an
// unsupported status falls back to a direct_response with that code instead of routing nowhere.
func TestCreateRedirectRoute(t *testing.T) {
	logger := createTestLogger()
	tr := NewTranslator(logger, testRouterConfig(), nil, testConfig())
	host := "example.org"

	// Supported status (301) → RedirectAction; redirect route still carries the :method match.
	supported := &models.Route{Method: "GET", Path: "/old", OperationPath: "/old",
		Redirect: &models.RouteRedirect{StatusCode: 301, Hostname: &host}}
	r := tr.createRedirectRoute("GET|/old|main", "GET", "/old", "/old", supported)
	require.NotNil(t, r.GetRedirect(), "supported status must yield a RedirectAction")
	assert.Equal(t, "example.org", r.GetRedirect().GetHostRedirect())
	assert.NotEmpty(t, r.GetMatch().GetHeaders(), "redirect route must carry the :method match")

	// Unsupported status (300) → direct_response fallback, no panic, no RedirectAction.
	unsupported := &models.Route{Method: "GET", Path: "/old", OperationPath: "/old",
		Redirect: &models.RouteRedirect{StatusCode: 300}}
	r = tr.createRedirectRoute("GET|/old|main", "GET", "/old", "/old", unsupported)
	assert.Nil(t, r.GetRedirect())
	require.NotNil(t, r.GetDirectResponse())
	assert.Equal(t, uint32(300), r.GetDirectResponse().GetStatus())
}
