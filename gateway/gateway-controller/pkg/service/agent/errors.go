...
// KindMismatchError is returned when the payload declares a kind other than Agent.
// Without this the wrong kind would be persisted from the /agents endpoint and
// the artifact would then be invisible to every Agent read path.
type KindMismatchError struct {
	Kind string
}

func (e *KindMismatchError) Error() string {
	return fmt.Sprintf("kind mismatch: expected 'Agent' but payload declares '%s'", e.Kind)
}
