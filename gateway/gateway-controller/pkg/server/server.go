...
// Package server contains the gateway-controller startup wiring.
// Call Run to boot the full server; pass a non-nil extensions slice to plug in
// additional REST API surfaces, event processors, and auth roles at build time.
package server
...
// Run boots the gateway-controller. configPath is the path to the TOML config
// file. extensions is a slice of build-time plugins; pass nil for the base
// gateway-controller with no extensions.
func Run(configPath string, extensions []extension.Extension) {
	...
	log.Info("EventHub initialized for multi-replica sync", slog.String("gateway_id", gatewayID))
	...
	for _, ext := range extensions {
		if err := ext.Initialize(svc); err != nil {
			log.Error("Extension initialization failed", slog.Any("error", err))
			os.Exit(1)
		}
	}
	...
	// Collect extension auth roles before building the middleware config.
	var extRoles map[string][]string
	if len(extensions) > 0 {
		extRoles = make(map[string][]string)
		for _, ext := range extensions {
			for k, v := range ext.AuthRoles() {
				extRoles[k] = v
			}
		}
	}
	authConfig := BuildAuthConfig(cfg, extRoles)
	...
	for _, ext := range extensions {
		ext.RegisterEventProcessors(evtListener, svc)
	}
	...
	// Let each extension register its own routes after core routes are up.
	for _, ext := range extensions {
		ext.RegisterRoutes(router, svc)
	}
...
// BuildAuthConfig constructs the auth middleware config from the gateway
// configuration and any additional route→roles mappings provided by
// build-time extensions.
func BuildAuthConfig(cfg *config.Config, extensionRoles map[string][]string) commonmodels.AuthConfig {
	...
	// Merge extension-provided roles.
	for k, v := range extensionRoles {
		relativeRoles[k] = v
	}
