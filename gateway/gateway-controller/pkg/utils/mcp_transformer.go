...
func addMCPSpecificOperations(mcpConfig *api.MCPProxyConfiguration, optionsRequired bool) []api.Operation {
	...
	// If no MCP specification version is declared, default to the oldest version
	// supported by this gateway. Explicitly declared versions are preserved as-is,
	// including versions not explicitly supported by this gateway.
	mcpSpecVersions := mcpConfig.Spec.EffectiveSpecVersions()
	if len(mcpSpecVersions) == 0 {
		mcpSpecVersions = []string{MIN_SUPPORTED_MCP_SPEC_VERSION}
	}
