...
func TestLLMProviderTransformer_TransformProxy_AdditionalProviderUsesContextAsBasePath(t *testing.T) {
	store := storage.NewConfigStore()
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	db := newTestSQLiteStorage(t, logger)
	...
	saveProvider := func(handle, context, uuid string) {
		source := api.LLMProviderConfiguration{
			ApiVersion: api.LLMProviderConfigurationApiVersionGatewayApiPlatformWso2Comv1,
			Kind:       api.LLMProviderConfigurationKindLlmProvider,
	...
	saveProvider("primary-provider", "/primary-context", "0000-primary-provider-000000000000")
	saveProvider("additional-provider", "/additional-context", "0000-additional-provider-00000000")

	transformer := NewLLMProviderTransformer(store, db,
		&config.RouterConfig{ListenerPort: 8080}, newTestPolicyVersionResolver())
	alias := "anthropic-provider"
	...
	proxy := &api.LLMProxyConfiguration{
		ApiVersion: api.LLMProxyConfigurationApiVersionGatewayApiPlatformWso2Comv1,
		Kind:       api.LLMProxyConfigurationKindLlmProxy,
		Metadata:   api.Metadata{Name: "multi-provider-proxy"},
		Spec: api.LLMProxyConfigData{
			DisplayName:         "multi-provider-proxy",
			Version:             "v1.0",
			Provider:            api.LLMProxyProvider{Id: "primary-provider"},
	...
	definition := (*result.Spec.UpstreamDefinitions)[0]
	assert.Equal(t, "anthropic-provider", definition.Name)
	require.NotNil(t, definition.BasePath)
	assert.Equal(t, "/additional-context", *definition.BasePath)
	require.Len(t, definition.Upstreams, 1)
	assert.Equal(t, "http://127.0.0.1:8080", definition.Upstreams[0].Url)
}
