...
func (l *EventListener) handleAPIKeyUpsert(event eventhub.Event) {
	...
	l.logger.Info("Processing API key upsert event",
		slog.String("action", event.Action),
		slog.String("api_id", apiID),
	...
	l.logger.Info("Successfully processed API key upsert event",
		slog.String("action", event.Action),
		slog.String("api_id", cfg.UUID),
		slog.String("api_key_id", keyID),
		slog.String("event_id", event.EventID))
...
func (l *EventListener) handleAPIKeyRevoke(event eventhub.Event) {
	...
	l.logger.Info("Processing API key revoke event",
		slog.String("api_id", apiID),
		slog.String("api_key_id", keyID),
		slog.String("event_id", event.EventID))
	...
	if l.apiKeyXDSManager != nil {
		if err := l.apiKeyXDSManager.RevokeAPIKey(cfg.UUID, apiName, apiVersion, apiKeyName, event.EventID); err != nil {
	...
	l.logger.Info("Successfully processed API key revoke event",
		slog.String("api_id", cfg.UUID),
		slog.String("api_key_id", keyID),
		slog.String("event_id", event.EventID))
}
