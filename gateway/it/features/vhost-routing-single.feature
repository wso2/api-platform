...
Feature: Gateway vhost routing with single-domain defaults
  ...
  Scenario: Sentinel vhost resolves to gateway defaults at deploy time
    When I deploy this API configuration:
      ...
      spec:
        displayName: VHost-Single-Sentinel
        version: v1.0
        context: /vhost-single-sentinel/$version
        vhosts:
      ...
      """
    Then the response should be successful
    And I wait for the endpoint "http://localhost:8080/vhost-single-sentinel/v1.0/whoami" to be ready with host "api.wso2.com"

    # Sentinel resolves to *.wso2.com — production traffic routed on api.wso2.com
    When I clear all headers
    And I set request host to "api.wso2.com"
    And I send a GET request to "http://localhost:8080/vhost-single-sentinel/v1.0/whoami"
    Then the response should be successful
    And the response should be valid JSON
    And the JSON response field "path" should be "/whoami"

    # Sandbox sentinel resolves to *-sandbox.wso2.com — sandbox traffic routed on api-sandbox.wso2.com
    When I clear all headers
    And I set request host to "api-sandbox.wso2.com"
    And I send a GET request to "http://localhost:8080/vhost-single-sentinel/v1.0/whoami"
    Then the response should be successful
