...
Feature: Model Round-Robin Multi-Provider Routing
  ...
  Scenario: Round-robin rotates across primary and two additional providers
    ...
    And I send a POST request to "http://localhost:8080/mp-rr-proxy/chat/completions" with body:
      """
  ...
  Scenario: Same model name on two providers is suspended independently
    ...
    When I set header "Content-Type" to "application/json"
    And I send a POST request to "http://localhost:8080/mp-susp-proxy/chat/completions" with body:
      """
      {"model": "original-model", "messages": [{"role": "user", "content": "Hello"}]}
      """
    Then the response status code should be 200
    ...
    When I set header "Content-Type" to "application/json"
    And I send a POST request to "http://localhost:8080/mp-susp-proxy/chat/completions?statusCode=429" with body:
      """
      {"model": "original-model", "messages": [{"role": "user", "content": "Hello"}]}
      """
