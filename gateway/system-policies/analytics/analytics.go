...
func (a *AnalyticsPolicy) OnRequestBody(_ context.Context, ctx *policy.RequestContext, params map[string]interface{}) policy.RequestAction {
	...
	case policy.APIKindMCP:
		...
		if !shouldParseBody(ctx.SharedContext) {
			break
		}
...
func extractMCPResponseAnalyticsProps(payload map[string]interface{}) *McpResponseAnalyticsProperties {
	...
	// Every field read above counts: a legacy initialize result may state the negotiated
	// protocol version without a serverInfo object, and dropping the record would lose it.
	if serverInfo.ProtocolVersion != "" || serverInfo.Name != "" || serverInfo.Version != "" ||
		len(serverInfo.SupportedVersions) > 0 {
