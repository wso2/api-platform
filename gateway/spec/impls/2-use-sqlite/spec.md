...
- **API Configuration**: Represents a complete API definition including metadata (ID, name, version, timestamps), upstream endpoints, operations, and policies. Stored across two tables: `deployments` (metadata) and `deployment_configs` (JSON payloads with configuration and source_configuration).
