...
FORCE=false
CERTS_ONLY=false
ALLOW_INSECURE_KEY_PERMS="${ALLOW_INSECURE_KEY_PERMS:-false}"
...
Private key permissions:
  Every generated private key (TLS keys, the AES-256 encryption key) is restricted to its
  owner plus a read-only ACL entry for the container's fixed UID. If setfacl is unavailable
  or fails, setup refuses to fall back to world-readable (644) permissions and exits with an
  error. Set ALLOW_INSECURE_KEY_PERMS=true to accept that reduced protection instead (e.g. on
  macOS, which has no setfacl) -- install the 'acl' package (setfacl) to avoid needing it.
...
restrict_secret_file() {
  ...
  if command -v setfacl >/dev/null 2>&1; then
    chmod 600 "$file"
    # -b strips any pre-existing ACL entries (e.g. inherited from the parent
    # directory's default ACL, or left over from an earlier run) before granting
    # the container UID read access -- chmod alone does not remove named ACL
    # entries, so without this a stale entry could leave another local user able
    # to read a private key.
    if setfacl -b "$file" 2>/dev/null && setfacl -m "u:${CONTAINER_UID}:r,m::r" "$file" 2>/dev/null; then
      return
    fi
    if [[ "$ALLOW_INSECURE_KEY_PERMS" != true ]]; then
      echo "error: setfacl failed to grant the container UID (${CONTAINER_UID}) read access to $file." >&2
      echo "       Refusing to fall back to world-readable (644) permissions on a private key." >&2
      echo "       Re-run with ALLOW_INSECURE_KEY_PERMS=true to accept the reduced protection." >&2
      exit 1
    fi
    log "  - WARNING: setfacl failed on $file; ALLOW_INSECURE_KEY_PERMS=true set, falling back to world-readable so the container (UID ${CONTAINER_UID}) can still read it"
    chmod "$FILE_MODE" "$file"
    return
  fi
  if [[ "$ALLOW_INSECURE_KEY_PERMS" != true ]]; then
    echo "error: setfacl is not installed, so $file cannot be restricted to the owner + container UID (${CONTAINER_UID}) only." >&2
    echo "       Refusing to fall back to world-readable (644) permissions on a private key." >&2
    echo "       Install the 'acl' package (setfacl), or re-run with ALLOW_INSECURE_KEY_PERMS=true to accept the" >&2
    echo "       reduced protection (e.g. on macOS, which has no setfacl)." >&2
    exit 1
  fi
  log "  - WARNING: setfacl not found; ALLOW_INSECURE_KEY_PERMS=true set, falling back to world-readable so the container (UID ${CONTAINER_UID}) can still read $file"
  chmod "$FILE_MODE" "$file"
}
...
gen_xds_certs() {
  if [[ "$FORCE" == false && -f "$CERTS_DIR/ca.crt" && -f "$CERTS_DIR/ca.key" \
        && -f "$CERTS_DIR/server.crt" && -f "$CERTS_DIR/server.key" \
        && -f "$CERTS_DIR/envoy-client.crt" && -f "$CERTS_DIR/envoy-client.key" \
        && -f "$CERTS_DIR/policy-engine-client.crt" && -f "$CERTS_DIR/policy-engine-client.key" ]]; then
