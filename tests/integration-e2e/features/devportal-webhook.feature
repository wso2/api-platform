Feature: Credentials issued in the API Portal authorize gateway invocation via webhook
  ...
  Scenario: A developer-portal API key and subscription propagate to the gateway and authorize a call
    ...
    When the subscription plan is synced to the API Portal
    And the API is published to the API Portal linked to the platform API
    And an application subscribed to the API is created in the API Portal
    And an API key is generated in the API Portal
    Then invoking the secured API through the gateway with the API Portal credentials returns 200
  ...
  Scenario: Developer-portal credential-lifecycle changes propagate to platform-api and the gateway
    ...
    Then an unauthenticated request to the secured API is rejected
    When the subscription plan is synced to the API Portal
    And a second subscription plan is synced to the API Portal
    And the API is published to the API Portal linked to the platform API
    And an application subscribed to the API is created in the API Portal
    And an API key is generated in the API Portal
    Then invoking the secured API through the gateway with the API Portal credentials returns 200
    ...
    When the applied subscription plan of the API is switched in the API Portal
    ...
    When the subscription token is regenerated in the API Portal
    ...
    When the subscription is paused in the API Portal
    Then invoking the secured API through the gateway is rejected
    When the subscription is resumed in the API Portal
    Then invoking the secured API through the gateway with the API Portal credentials returns 200
    # Remove the subscription (terminal); the gateway stops honoring the credentials.
    When the subscription is removed in the API Portal
    Then invoking the secured API through the gateway is rejected
