...
const (
	...
	// Webhook trust material shared with the developer portal. The secret matches
	// WEBHOOK_SECRET in docker-compose.yaml; the key pair is the committed dev
	// fixture (the .pub is registered on the devportal subscriber, the .pem is
	// mounted into platform-api — verified to be a matching RSA pair).
	webhookSecret       = "5bd108b058ac9b318faf771c82a3f88bf6d3be5cc51c221e7ee213dabdbdee22"
	webhookKeySource    = "../../portals/developer-portal/devportal-webhook.pem"
	webhookPubKeySource = "../../portals/developer-portal/devportal-webhook.pub"
...
func prepareWebhookKey() error {
	data, err := os.ReadFile(webhookKeySource)
	if err != nil {
		return fmt.Errorf("read webhook private key %s: %w", webhookKeySource, err)
	}
	cwd, err := os.Getwd()
	if err != nil {
		return err
	...
	if err := os.WriteFile(dst, data, 0o644); err != nil {
		return fmt.Errorf("write webhook private key copy: %w", err)
	}
