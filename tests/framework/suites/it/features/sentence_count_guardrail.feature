...
Feature: Sentence Count Guardrail
  ...
  Scenario: Block request when sentence count exceeds maximum
    ...
    And I send a "GET" request to "/sentence-count-max/v1.0/health" until status 200

    # Request with 2 sentences - should pass (within limit)
    When I send a "POST" request to "/sentence-count-max/v1.0/validate" with body:
      """
  ...
  Scenario: Block request when sentence count is below minimum
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-min/v1.0/health" until status 200

    # Request with 1 sentence - should be blocked (below min of 3)
    When I send a "POST" request to "/sentence-count-min/v1.0/validate" with body:
      """
  ...
  Scenario: Validate sentence count using JSONPath extraction
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-jsonpath/v1.0/health" until status 200

    # JSON with message field under 5 sentences - should pass
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/sentence-count-jsonpath/v1.0/chat" with body:
      """
      {
  ...
  Scenario: Validate sentence count using nested JSONPath
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-nested/v1.0/health" until status 200

    # Nested JSON with content field under 3 sentences - should pass
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/sentence-count-nested/v1.0/chat" with body:
      """
      {
        "data": {
  ...
  Scenario: Handle invalid JSONPath gracefully
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-invalid-path/v1.0/health" until status 200

    # JSON without the expected path - should return error
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/sentence-count-invalid-path/v1.0/validate" with body:
      """
      {
  ...
  Scenario: Inverted logic blocks content within the range
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-invert/v1.0/health" until status 200

    # Request with 3 sentences (within 2-4 range) - should FAIL because invert=true
    When I send a "POST" request to "/sentence-count-invert/v1.0/validate" with body:
      """
  ...
  Scenario: Show detailed assessment in error response
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-assessment/v1.0/health" until status 200

    # Request that fails - should include assessment details
    When I send a "POST" request to "/sentence-count-assessment/v1.0/validate" with body:
      """
      First. Second. Third. Fourth. This exceeds the max of two sentences!
      """
    Then the response status code should be 422
    And the response should be valid JSON
    And the JSON response field "message.assessments" should be "Violation of sentence count detected. Expected sentence count to be between 1 and 2 sentences."
  ...
  Scenario: Empty request body is handled correctly
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-empty/v1.0/health" until status 200

    # Empty body has 0 sentences - should fail (below min of 1)
    When I send a "POST" request to "/sentence-count-empty/v1.0/validate" with body:
      """
      """
    Then the response status code should be 422
  ...
  Scenario: Exact boundary values are accepted
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-boundary/v1.0/health" until status 200

    # Exactly 2 sentences (min boundary) - should pass
    When I send a "POST" request to "/sentence-count-boundary/v1.0/validate" with body:
      """
  ...
  Scenario: Combined request and response validation
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-combined/v1.0/health" until status 200

    # Request within limit - should pass request validation
    When I send a "POST" request to "/sentence-count-combined/v1.0/validate" with body:
      """
  ...
  Scenario: Handle multiple punctuation marks correctly
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-punctuation/v1.0/health" until status 200

    # Multiple exclamation marks - "Hello!!! World???" should count based on [.!?] splitting
    When I send a "POST" request to "/sentence-count-punctuation/v1.0/validate" with body:
      """
      Hello!!! World???
      """
    Then the response status code should be 200
  ...
  Scenario: Handle plain text content type
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-plaintext/v1.0/health" until status 200

    # Plain text content - should count sentences correctly
    When I set header "Content-Type" to "text/plain"
    And I send a "POST" request to "/sentence-count-plaintext/v1.0/validate" with body:
      """
  ...
  Scenario: Verify complete error response structure
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/sentence-count-error-structure/v1.0/health" until status 200

    # Trigger error and verify full response structure
    When I send a "POST" request to "/sentence-count-error-structure/v1.0/validate" with body:
      """
