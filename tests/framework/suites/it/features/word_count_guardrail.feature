...
Feature: Word Count Guardrail
  ...
  Scenario: Block request when word count exceeds maximum
    ...
    And I send a "GET" request to "/word-count-max/v1.0/health" until status 200

    # Request with 5 words - should pass (within limit)
    When I send a "POST" request to "/word-count-max/v1.0/validate" with body:
      """
  ...
  Scenario: Block request when word count is below minimum
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-min/v1.0/health" until status 200

    # Request with 2 words - should be blocked (below min of 5)
    When I send a "POST" request to "/word-count-min/v1.0/validate" with body:
      """
      Too short
      """
    Then the response status code should be 422
  ...
  Scenario: Validate word count using JSONPath extraction
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-jsonpath/v1.0/health" until status 200

    # JSON with message field under 20 words - should pass
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/word-count-jsonpath/v1.0/chat" with body:
      """
      {
  ...
  Scenario: Inverted logic blocks content within the range
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-invert/v1.0/health" until status 200

    # Request with 7 words (within 5-10 range) - should FAIL because invert=true
    When I send a "POST" request to "/word-count-invert/v1.0/validate" with body:
      """
  ...
  Scenario: Show detailed assessment in error response
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-assessment/v1.0/health" until status 200

    # Request that fails - should include assessment details
    When I send a "POST" request to "/word-count-assessment/v1.0/validate" with body:
      """
      This message has way more than five words and should fail
      """
    Then the response status code should be 422
    And the response should be valid JSON
    And the JSON response field "message.assessments" should be "Violation of word count detected. Expected word count to be between 1 and 5 words."
  ...
  Scenario: Empty request body is handled correctly
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-empty/v1.0/health" until status 200

    # Empty body has 0 words - should fail (below min of 1)
    When I send a "POST" request to "/word-count-empty/v1.0/validate" with body:
      """
      """
    Then the response status code should be 422
  ...
  Scenario: Exact boundary values are accepted
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-boundary/v1.0/health" until status 200

    # Exactly 5 words (min boundary) - should pass
    When I send a "POST" request to "/word-count-boundary/v1.0/validate" with body:
      """
  ...
  Scenario: Combined request and response validation
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-combined/v1.0/health" until status 200

    # Request within limit - should pass request validation
    When I send a "POST" request to "/word-count-combined/v1.0/validate" with body:
      """
  ...
  Scenario: Validate word count using nested JSONPath
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-nested/v1.0/health" until status 200

    # Nested JSON with content field under 10 words - should pass
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/word-count-nested/v1.0/chat" with body:
      """
      {
        "data": {
  ...
  Scenario: Handle invalid JSONPath gracefully
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-invalid-path/v1.0/health" until status 200

    # JSON without the expected path - should return error
    When I set header "Content-Type" to "application/json"
    And I send a "POST" request to "/word-count-invalid-path/v1.0/validate" with body:
      """
      {
  ...
  Scenario: Handle punctuation and special characters correctly
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-punctuation/v1.0/health" until status 200

    # Punctuation should not affect word count - "Hello... world!!!" = 2 words
    When I send a "POST" request to "/word-count-punctuation/v1.0/validate" with body:
      """
  ...
  Scenario: Handle plain text content type
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-plaintext/v1.0/health" until status 200

    # Plain text content - should count words correctly
    When I set header "Content-Type" to "text/plain"
    And I send a "POST" request to "/word-count-plaintext/v1.0/validate" with body:
      """
  ...
  Scenario: Verify complete error response structure
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/word-count-error-structure/v1.0/health" until status 200

    # Trigger error and verify full response structure
    When I send a "POST" request to "/word-count-error-structure/v1.0/validate" with body:
      """
