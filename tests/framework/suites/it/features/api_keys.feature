...
Feature: API Key Management Operations
  ...
  Scenario: Complete API key lifecycle - generate, list, regenerate, and revoke
    ...
    And the JSON response field "apiKey.name" should be "test-key-1"
    ...
    And the response should be valid JSON
    And the JSON response field "status" should be "success"
    And the JSON response field "apiKeys[0].name" should be "test-key-1"
  ...
  Scenario: Generate multiple API keys for same API
    ...
    Then the response status code should be 200
    And the response should be valid JSON
    And the JSON response field "apiKeys[1].name" should be "key-one"
    And the JSON response field "apiKeys[0].name" should be "key-two"
