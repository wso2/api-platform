...
Feature: Search Deployments API
  ...
  Scenario: Search APIs by displayName filter
    ...
    And the response should be valid JSON
    And the JSON response field "status" should be "success"
    And the JSON response field "apis[0].spec.displayName" should be "UniqueDisplayName"
    # Cleanup
  ...
  Scenario: Search APIs by status filter
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/status-search/test" until status 200
  ...
  Scenario: Search MCP proxies with no filters
    ...
    And the response should be valid JSON
    And the JSON response field "status" should be "success"
    And the JSON response should have field "mcpProxies"
    # Cleanup
  ...
  Scenario: Search MCP proxies by displayName filter
    ...
    And the response should be valid JSON
    And the JSON response field "status" should be "success"
    And the JSON response field "mcpProxies[0].spec.displayName" should be "UniqueMCPDisplayName"
    # Cleanup
