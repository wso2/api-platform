...
Feature: Dynamic Endpoint policy
  ...
  Scenario: Operation routed to a named upstream definition while others use the default upstream
    ...
    And I send a "GET" request to "/dynamic-endpoint/v1.0/ping" until status 200
    ...
    And I send a "GET" request to "/dynamic-endpoint/v1.0/whoami"
    ...
    When I reset the request
    And I send a "GET" request to "/dynamic-endpoint/v1.0/ping"
    Then the response status code should be 200
    And the response should be valid JSON
  ...
  Scenario: Different operations route to upstreams with different base paths
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/dynamic-endpoint-routes/v1.0/items" until status 200

    # Routed to foo-upstream: base path /foo prepended.
    When I reset the request
    And I send a "GET" request to "/dynamic-endpoint-routes/v1.0/items"
    Then the response status code should be 200
    And the response should be valid JSON
    ...
    When I reset the request
    And I send a "GET" request to "/dynamic-endpoint-routes/v1.0/records"
    Then the response status code should be 200
    And the response should be valid JSON
    ...
    When I reset the request
    And I send a "GET" request to "/dynamic-endpoint-routes/v1.0/extras"
    Then the response status code should be 200
    And the response should be valid JSON
  ...
  Scenario: Deploy fails when targetUpstream is omitted
    ...
    And the JSON response field "errors[0].message" should be "targetUpstream is required"
  ...
  Scenario: Dynamic-endpoint policy applies on the sandbox vhost
    ...
    And I send a "GET" request to "/dynamic-endpoint-sandbox/v1.0/whoami" until status 200
    And the response should be valid JSON
    And the JSON response field "path" should be "/alternate/whoami"
    ...
    And I send a "GET" request to "/dynamic-endpoint-sandbox/v1.0/whoami"
    Then the response status code should be 200
    And the response should be valid JSON
    And the JSON response field "path" should be "/alternate/whoami"
  ...
  Scenario: API-level dynamic-endpoint policy applies on the sandbox vhost
    ...
    And I send a "GET" request to "/dynamic-endpoint-api-sandbox/v1.0/whoami" until status 200
    And the response should be valid JSON
    And the JSON response field "path" should be "/alternate/whoami"
    ...
    And I send a "GET" request to "/dynamic-endpoint-api-sandbox/v1.0/ping"
    Then the response status code should be 200
    And the response should be valid JSON
  ...
  Scenario: Sandbox operation without the policy falls back to the sandbox upstream
    ...
    And I send a "GET" request to "/dynamic-endpoint-sandbox-mixed/v1.0/ping" until status 200

    # Sandbox vhost, operation WITH the policy: diverted to alt-upstream (base /alternate).
    When I reset the request
    And I set request host to "dyn-mixed-sandbox.local"
    And I send a "GET" request to "/dynamic-endpoint-sandbox-mixed/v1.0/whoami"
    Then the response status code should be 200
    And the response should be valid JSON
    And the JSON response field "path" should be "/alternate/whoami"
    ...
    When I reset the request
    And I set request host to "dyn-mixed-sandbox.local"
    And I send a "GET" request to "/dynamic-endpoint-sandbox-mixed/v1.0/ping"
    Then the response status code should be 200
    And the response should be valid JSON
  ...
  Scenario: Dynamic-endpoint combined with a path-rewrite policy on the same operation
    ...
    When I create API with configuration:
      ...
      """
    Then the response status code should be 201
    And I send a "GET" request to "/dynamic-endpoint-rewrite/v1.0/whoami" until status 200

    # Both the rewrite (/whoami -> /rewritten) and the dynamic-endpoint base (/alternate) apply,
    # prefixed exactly once.
    When I reset the request
    And I send a "GET" request to "/dynamic-endpoint-rewrite/v1.0/whoami"
    Then the response status code should be 200
    And the response should be valid JSON
  ...
  Scenario: Dynamic-endpoint and a path-rewrite policy combined on the sandbox vhost
    ...
    And I send a "GET" request to "/dynamic-endpoint-rewrite-sandbox/v1.0/whoami" until status 200
    And the response should be valid JSON
    And the JSON response field "path" should be "/alternate/rewritten"
