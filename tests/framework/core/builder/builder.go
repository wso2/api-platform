...
const (
	...
	// BrowserJSCoverage identifies JavaScript coverage collected from a Chromium page.
	BrowserJSCoverage CoverageType = "browser-js"
)

// BrowserCoverageSpec describes browser-side source files and report output.
...
func validateCoverage(spec CoverageSpec) error {
	...
	for _, coverageType := range spec.Types {
		if coverageType != GoCoverage && coverageType != NodeV8Coverage && coverageType != BrowserJSCoverage {
	...
	if containsCoverageType(spec.Types, BrowserJSCoverage) {
		if err := validateBrowserCoverage(spec.Browser); err != nil {
			return err
		}
	}
	return nil
...
func containsCoverageType(types []CoverageType, wanted CoverageType) bool {
	for _, coverageType := range types {
		if coverageType == wanted {
			return true
		}
	}
	return false
...
func validateBrowserCoverage(spec BrowserCoverageSpec) error {
	if !spec.Supported {
		return fmt.Errorf("browser coverage must be marked supported when browser-js is declared")
	}
	if strings.TrimSpace(spec.SourceRoot) == "" {
		return fmt.Errorf("browser coverage source root is required")
	}
	if filepath.IsAbs(spec.SourceRoot) {
		return fmt.Errorf("browser coverage source root must be repository-relative")
	}
	if strings.TrimSpace(spec.OutputDir) == "" {
		return fmt.Errorf("browser coverage output directory is required")
	}
	if !filepath.IsAbs(spec.OutputDir) || filepath.Clean(spec.OutputDir) != spec.OutputDir {
		return fmt.Errorf("browser coverage output directory must be an absolute clean path")
	}
	return nil
}
