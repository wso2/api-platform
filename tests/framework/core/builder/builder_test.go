...
func TestBuildAcceptsBrowserCoverageSpec(t *testing.T) {
	root := t.TempDir()
	err := Build(context.Background(), Spec{
		Component: "portal",
		SourceDir: "portal",
		Images:    []Image{{Name: "portal:test", Dockerfile: "portal/Dockerfile", Context: "portal"}},
		Coverage: CoverageSpec{
			Supported: true,
			Types:     []CoverageType{BrowserJSCoverage},
			OutputDir: "/coverage",
		...
		Plan: func(_ string, _ string, _ CoverageSpec) ([]Command, error) {
			return []Command{{Directory: root, Args: []string{"true"}}}, nil
		},
	}, Request{RepoRoot: root, Version: "test", Coverage: true, Runner: &recordingRunner{}})
	require.NoError(t, err)
}

func TestBuildRejectsIncompleteBrowserCoverageSpec(t *testing.T) {
	err := Build(context.Background(), Spec{
		Component: "portal",
		SourceDir: "portal",
		Images:    []Image{{Name: "portal:test", Dockerfile: "portal/Dockerfile", Context: "portal"}},
		Coverage: CoverageSpec{
			...
			Types:     []CoverageType{BrowserJSCoverage},
			OutputDir: "/coverage",
			Browser:   BrowserCoverageSpec{Supported: true, OutputDir: "/coverage/browser"},
		},
		Plan: func(string, string, CoverageSpec) ([]Command, error) { return nil, nil },
	}, Request{RepoRoot: t.TempDir(), Version: "test", Runner: &recordingRunner{}})
	require.ErrorContains(t, err, "browser coverage source root is required")
}
