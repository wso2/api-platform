...
// context integration. Register records a scenario-scoped resource, while RegisterRunner
// records a runner-scoped resource. The engine sweeps the two scopes independently after
// each scenario and when the runner finishes. A missing registry is a wiring error; an
// invalid or nil registry cannot be installed.
//
