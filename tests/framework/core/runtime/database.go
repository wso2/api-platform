...
func NewCredentials() (Credentials, error) {
	...
	// Use a shell- and DSN-safe alphabet with the complexity required by SQL Server.
...
// engine encapsulates engine-specific database provisioning.
type engine interface {
	// serverDefinition builds the database server definition.
	serverDefinition(creds Credentials, stores []*Store, repoRoot string, image ...components.ImageRef) (*components.Definition, error)

	// canBootstrapAtStart reports whether the image applies databases and schemas at startup.
	canBootstrapAtStart() bool

	// bootAttempts reports how many startup attempts are permitted.
	bootAttempts() int

	// dsn builds the connection coordinates for a store.
	dsn(store *Store, alias string, creds Credentials) components.DSN

	// applySchema creates databases and applies schemas after startup when required.
	applySchema(
...
// bootstrapScript generates the database and schema initialization script.
...
// applySchema is a no-op because PostgreSQL applies the schema during startup.
...
// writeTempFile stages generated content for a container file mount.
...
func (sqlServerEngine) serverDefinition(
	...
	// Validate schema paths before starting the server.
	for _, s := range stores {
		for _, ddl := range s.Schema {
...
// splitBatches splits a script on the GO separator used by sqlcmd.
...
// Request describes one component's storage need within a block.
type Request struct {
...
type Store struct {
	...
	// Database is the generated physical name.
	Database string
...
// Plan is the resolved storage layout of one block.
type Plan struct {
...
//
// BuildPlan allocates one store per owning component instance, resolves shared stores,
// and selects one server per non-embedded engine.
func BuildPlan(requests []Request) (*Plan, error) {
	...
	// Create owner stores before resolving sharers.
...
// databaseName converts a component name into a legal database identifier.
...
// launchWithRetry starts a database server, retrying failed startup attempts.
